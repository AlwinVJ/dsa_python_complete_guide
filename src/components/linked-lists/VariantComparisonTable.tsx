...
const ROWS: { axis: string; values: [string, string, string, string] }[] = [
  { axis: "Structure",
    values: ["val · next", "val · prev · next", "val · next (tail→head)", "val · prev · next (ring)"] },
  { axis: "Memory / node",
    values: ["1 pointer", "2 pointers", "1 pointer", "2 pointers"] },
  { axis: "Traversal",
    values: ["Forward only", "Forward + backward", "Forward, wraps around", "Both, wraps around"] },
  { axis: "Insert at head",
    values: ["O(1)", "O(1)", "O(1)*", "O(1)"] },
  { axis: "Insert at tail",
    values: ["O(1) w/ tail", "O(1)", "O(1) w/ tail", "O(1)"] },
  { axis: "Delete at head",
    values: ["O(1)", "O(1)", "O(1)*", "O(1)"] },
  { axis: "Delete at tail",
    values: ["O(n)", "O(1)", "O(n)", "O(1)"] },
  { axis: "Reverse traversal",
    values: ["✗", "✓", "✗", "✓"] },
  { axis: "Advantage",
    values: [
        ...
        <tbody>
          {ROWS.map((r) => (
            <tr key={r.axis} className="border-t border-border">
              ...
              {r.values.map((c, i) => (
                <td key={i} className="px-3 py-2 text-muted-foreground">{c}</td>
              ))}
            </tr>
          ))}
        </tbody>
