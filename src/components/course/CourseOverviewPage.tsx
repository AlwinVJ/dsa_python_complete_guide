...
export function CourseOverviewPage({ slug }: { slug: string }) {
  const course = getCourse(slug);

  if (!course) {
    return <div className="p-8 text-center text-sm text-muted-foreground">Course not found.</div>;
  }
      ...
      <div className="mt-10 space-y-10">
        {course.courseLayout === "overview" && (
          <div>
            <SectionHeader
              title="Learning Path"
              tagline={`Master ${course.title.toLowerCase()} step-by-step through our structured curriculum.`}
            />
          </div>
        )}
        ...
        {variants.length > 0 && (
          <div>
            <SectionHeader
              title="Variants"
              tagline={`Complete mini-courses for each ${course.title.toLowerCase()} variant.`}
