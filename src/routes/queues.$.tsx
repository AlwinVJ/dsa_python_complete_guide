...
import { QueuePlayground } from "@/components/QueuePlayground";
import { CoursePrevNext } from "@/components/CoursePrevNext";
import { useLessonProgress } from "@/lib/lesson-progress";
import type { QueueLesson, QueueSection, QueueQuizItem } from "@/lib/queues/types";
...
export const Route = createFileRoute("/queues/$")({
  ...
  component: QueueLessonPage,
});

function QueueLessonPage() {
  ...
  const done = isDone("queues", progressKey);
  const diffColor =
    ...
    <PageShell>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="-mt-6 mb-8 flex flex-wrap items-center gap-3 text-sm">
          ...
          <button
            onClick={() => toggle("queues", progressKey)}
...
function SectionRenderer({ s }: { s: QueueSection }) {
  switch (s.type) {
    case "heading":
...
function QuizCard({ q }: { q: QueueQuizItem }) {
