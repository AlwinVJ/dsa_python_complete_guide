...
export const Route = createFileRoute("/array-vs-list")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "How a Python list differs from a traditional fixed-size array.",
      },
    ],
  }),
  component: Page,
  ...
  return (
    <PageShell>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="card-surface p-4">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Same type, fixed size, values stored inline.
            </div>
          </div>
          <div className="card-surface p-4">
            <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Any type, dynamic size, slots hold pointers.
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section title="Under the hood: pointers, not values">
        <p>
          A Python list is essentially an array of <b>pointers</b>. Each slot in the list references
          an object stored somewhere else in memory. That's why a single list can hold an integer, a
          string, and another list at the same time.
        </p>
...
└───────────┘            └──────┴──────┴──────┴──────┘`}
        ...
        <CodeBlock
          code={`mixed = [10, "hi", 3.14, [1, 2, 3]]\nfor item in mixed:\n    print(type(item))`}
        />
      </Section>

      <Callout kind="perf">
        Because slots are pointer-sized, indexing is still <b>O(1)</b> — the interpreter just jumps
        to the correct pointer and dereferences it.
      </Callout>
