  ...
  head: () => ({
    meta: [
      { title: "Graphs — Interactive Course — DSA with Python" },
      {
        name: "description",
        content:
          "Master graphs end-to-end: foundations, types, representations, traversals, and every core algorithm — with interactive visualizers and complete Python implementations.",
      },
      { property: "og:title", content: "Graphs — Interactive Course — DSA with Python" },
      {
        property: "og:description",
        content:
          "The flagship graphs course: foundations → types → representations → traversals → algorithms → review.",
      },
    ],
  }),
...
const tiers = [
  {
    ...
    icon: BookOpen,
    to: "/graphs/foundations/introduction" as const,
    count: G_FOUNDATIONS.length,
    ready: true,
  },
  {
    title: "Graph Types",
    tagline: "15 mini-courses — directed, weighted, bipartite, DAG, and more.",
    icon: Layers,
    ready: false,
  },
  {
    title: "Representations",
    tagline: "Adjacency list vs matrix vs edge list vs CSR.",
    icon: Network,
    to: "/graphs/representations/overview" as const,
    count: G_REPRESENTATIONS.length,
    ready: true,
  },
  {
    title: "Traversals",
    tagline: "BFS, DFS, iterative vs recursive, components.",
    icon: Compass,
    to: "/graphs/traversals/overview" as const,
    count: G_TRAVERSALS.length,
    ready: true,
  },
  {
    title: "Algorithms",
    tagline: "Shortest path, MST, topological sort, Union-Find.",
    icon: Zap,
    to: "/graphs/algorithms/overview" as const,
    count: G_ALGORITHMS.length,
    ready: true,
  },
  {
    title: "Review & Practice",
    tagline: "Cheatsheet, FAQ, interview bank, LC roadmap, quiz.",
    icon: RouteIcon,
    ready: false,
  },
];
  ...
  return (
    <PageShell>
      ...
      <section className="mb-8 grid gap-3 sm:grid-cols-3">
        <Stat
          label="Total lessons"
          value={`${G_FOUNDATIONS.length + G_REPRESENTATIONS.length + G_TRAVERSALS.length + G_ALGORITHMS.length}+`}
        />
      ...
      <section className="mb-10">
        ...
        <ul className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
          {[
            ...
            <li key={i} className="card-surface p-3">
              {s}
            </li>
          ))}
        </ul>
      ...
      <section className="mb-10">
        ...
        <p className="text-sm text-muted-foreground">
          Every major software system leans on graphs — routing (Google Maps), recommendations
          (Instagram, TikTok), dependency resolution (npm, pip), scheduling (build systems), and
          search (Google's link graph). Learn graphs once and you have the mental model for almost
          every non-trivial systems problem.
        </p>
      </section>
            ...
            <div key={t.title} className="card-surface flex items-start gap-3 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                {t.ready && t.to && (
                  <Link
                    to={t.to}
                    className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-[color:var(--brand)]"
                  >
      ...
      <section className="mb-10">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          The 22 Foundations lessons, in order. Complete them before moving to Types.
        </p>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-3 text-[11px] text-muted-foreground">
                    <span className="inline-flex items-center gap-1">
                      <Clock className="h-3 w-3" /> {l.readMinutes} min
                    </span>
      ...
      <section className="mb-10">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Build any graph you like — add vertices, connect them, drag nodes, toggle directions and
          weights. Live stats update every step.
        </p>
        <GraphPlayground />
      </section>
      ...
      <section className="mb-10">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Link
            to="/graphs/$"
            params={{ _splat: "foundations/introduction" }}
            className="card-surface p-3 hover:border-[color:var(--brand)]/60"
          >
