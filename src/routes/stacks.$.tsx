...
import type { StackLesson, StackSection, StackQuizItem } from "@/lib/stacks/types";
...
type Tier = "foundations" | "implementations" | "applications" | "revision";

const TIERS: Record<Tier, { title: string; lessons: StackLesson[] }> = {
  foundations:     { title: "Foundations",              lessons: STACK_FOUNDATIONS },
  implementations: { title: "Implementations",          lessons: STACK_IMPLEMENTATIONS },
  applications:    { title: "Algorithms & Applications", lessons: STACK_APPLICATIONS },
  revision:        { title: "Review & Practice",        lessons: STACK_REVISION },
};

type Resolved = { tier: Tier; tierTitle: string; lesson: StackLesson };
...
export const Route = createFileRoute("/stacks/$")({
  ...
  head: ({ params }) => {
    ...
    const title = r
      ? `${r.lesson.title} — Stacks — DSA with Python`
      : "Stacks — DSA with Python";
    return {
      meta: [
        { title },
        { name: "description", content: r?.lesson.description ?? "Stacks in Python — animated, interactive lessons." },
        { property: "og:title", content: title },
        { property: "og:description", content: r?.lesson.description ?? "" },
      ],
    };
  },
  component: StackLessonPage,
});

function StackLessonPage() {
  ...
  const done = isDone("stacks", progressKey);
  const diffColor =
    ...
    <PageShell>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="-mt-6 mb-8 flex flex-wrap items-center gap-3 text-sm">
          ...
          <button
            onClick={() => toggle("stacks", progressKey)}
          ...
          <Link to="/learn/$course" params={{ course: "stacks" }}
      ...
      <CoursePrevNext courseSlug="stacks" lessonSlug={progressKey} />
    </PageShell>
  );
}

function SectionRenderer({ s }: { s: StackSection }) {
  switch (s.type) {
    case "heading":
    ...
    case "playground":
      return (
        <div>
          ...
          <StackPlayground initial={s.initial} />
        </div>
      );
    case "complexity":
      return (
...
function QuizCard({ q }: { q: StackQuizItem }) {
