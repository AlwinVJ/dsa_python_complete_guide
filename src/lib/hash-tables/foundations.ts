...
export const HT_FOUNDATIONS: HTLesson[] = [
  {
    ...
    description:
      "A tour of what a hash table is, why every language ships one, and where this course is heading.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "A hash table stores key/value pairs and answers 'is this key here?' in constant time on average. Python's `dict` and `set` are hash tables, and so is nearly every cache, symbol table, and lookup index you have ever used.",
      },
      {
        type: "theory",
        bullets: [
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Given a username, how do you find its user record? An array can only look up by index (O(1)) or by scanning every element (O(n)). Sorted arrays add binary search (O(log n)) — still not free. Hash tables give you look-up by identity in O(1) on average.",
      },
      {
        type: "code",
        title: "phone-book lookup",
        code: `# array of tuples — O(n) per lookup
...
book["alice"]                    # instant`,
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "A key/value store maps unique keys to associated values. The key is the search handle; the value is the payload. The store must support put(k, v), get(k), and delete(k) — nothing else is fundamental.",
      },
      {
        type: "code",
        title: "the KV contract",
        code: `put(k, v)   # store v under key k, overwriting any existing entry
...
contains(k) # bool`,
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "'Dictionary' is the ADT name — an unordered collection of key/value pairs. 'Hash table' is one particular implementation of that ADT. Python calls its implementation `dict`; internally it is a hash table.",
      },
      {
        type: "callout",
        kind: "info",
        title: "Other implementations exist",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "A set stores unique keys with no associated values. The underlying implementation is still a hash table, but each bucket carries only the key, not a payload — so a set costs less memory and answers `in` in O(1).",
      },
      {
        type: "code",
        title: "when to reach for set",
        code: `# deduplicating
uniq = set(seen_ids)
...
if user_id in banned:   # set → O(1) · list → O(n)
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "At the metal, a hash table is a plain contiguous array. Each cell — a bucket — either holds an entry, a chain of entries, or a tombstone. The array is over-allocated so most slots are empty, which keeps collisions rare.",
      },
      {
        type: "buckets",
        capacity: 8,
        ...
        showLoadFactor: true,
        caption: "Four entries in an 8-slot table → load factor 0.50.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "A hash table is O(1) on average — assuming keys spread uniformly across buckets. When every key lands in the same bucket, the table degenerates into a linked list and every op costs O(n).",
      },
      {
        type: "callout",
        kind: "warn",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Space is O(n) in the number of entries, but the constant factor is bigger than an array because the table is intentionally over-sized. Typical CPython dicts stay at load factor ≤ 0.66 — so a dict of 100 keys occupies ~150 slots.",
      },
    ],
  },
  {
