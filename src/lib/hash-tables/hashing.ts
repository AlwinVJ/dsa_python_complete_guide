...
export const HT_HASHING: HTLesson[] = [
  {
    ...
    eyebrow: "Hashing · 1",
    description: "Hashing is the technique that turns arbitrary data into a fixed-size fingerprint. Everything a hash table does is built on it.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Hashing is the process of mapping a key of any size into an integer within a fixed range. That integer is the key's fingerprint. Two identical keys must produce the same fingerprint; two different keys usually — but not always — produce different ones." },
      { type: "hashFlow", key: "cat", hashValue: 3141592, bucket: 0, capacity: 8, caption: "The key 'cat' becomes hash 3141592, which lands in bucket 3141592 % 8 = 0." },
    ],
  },
  {
    slug: "why-hashing",
    title: "Why Hashing?",
    eyebrow: "Hashing · 2",
    description: "The trick that lets you look up a needle in a haystack without touching most of the hay.",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Sequential search must compare the query with each element. Hashing computes the location *from the key itself* — you jump directly there without comparing anything else." },
      { type: "callout", kind: "did", title: "One-shot look-up",
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "A hash function h(k) accepts any key from the key universe U and returns an integer, usually reduced modulo the table capacity m to produce a bucket index." },
      { type: "code", title: "the shape of a hash function", code:
`def h(key: str, m: int) -> int:
    return sum(ord(c) for c in key) % m

h("dog", 8)   # 3
h("god", 8)   # 3   ← collision by construction` },
      { type: "callout", kind: "warn", title: "Bad functions are worse than no function",
  ...
  },
  {
    ...
    sections: [
      { type: "code", code:
`def h(k, m): return k % m

h(1234, 11)   # 2
h(5678, 11)   # 3` },
      { type: "callout", kind: "warn", title: "Choose m carefully",
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "h(k) = ⌊m · (k·A mod 1)⌋. Choose A ≈ (√5 − 1)/2 (Knuth's suggestion) for a well-distributed output. Table size m does not need to be prime." },
      { type: "code", code:
`import math
...
def h(k, m): return int(m * ((k * A) % 1))` },
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Square k, extract r middle digits/bits from the result, use them as the hash. Every input bit affects the middle, so the output mixes well — but multiplication of large keys is expensive." },
    ],
  },
  {
    slug: "folding-method",
...
fold(123_456_789, 100)   # 68  →  123 + 456 + 789 = 1368 → 68` },
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "If the keys share a common prefix (student IDs like 2024xxxx), drop the shared digits and hash on the varying ones. Cheap and effective when the key structure is known." },
    ],
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "A universal family H of hash functions has the property that, for any two distinct keys x ≠ y, Pr[h(x) = h(y)] ≤ 1/m when h is chosen uniformly from H. Picking h at run time makes it impossible for an attacker to craft colliding inputs ahead of time." },
      { type: "callout", kind: "info", title: "Where Python uses it",
...
`def poly_hash(s, p=31, m=10**9 + 7):
    ...
    for ch in s:
        h = (h * p + ord(ch)) % m
    return h` },
      { type: "callout", kind: "did", title: "Why prime base",
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "For a substring, when you add a new right character and drop the left one, you don't recompute the whole polynomial — you subtract the leftmost term and multiply by the base. Foundational to Rabin-Karp string matching." },
      { type: "code", code:
...
new_hash = ((old_hash - ord(left) * p_pow) * p + ord(right)) % m` },
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "When the set of keys is known in advance and never changes, we can construct a two-level hash scheme (FKS) that guarantees O(1) worst-case look-up. Widely used in compilers for keyword lookup." },
    ],
  },
  {
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "code", title: "CPython (SipHash-2-4, simplified)", code:
`# real code lives in Python/pyhash.c
...
hash(2**61)    # 1    ← Mersenne prime wrap-around` },
      { type: "callout", kind: "did", title: "Modular identity",
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Load factor α = n / m, where n is the number of stored entries and m is the number of buckets. Low α → few collisions → fast operations. High α → many collisions → slow." },
      { type: "buckets", capacity: 8, buckets: [
        [{ key: "a" }], [{ key: "b" }], [{ key: "c" }], null,
        [{ key: "d" }], null, [{ key: "e" }], [{ key: "f" }],
      ], showLoadFactor: true, caption: "6 keys in 8 buckets → α = 0.75. Time to resize." },
    ],
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "The key universe is much larger than the table (usernames = infinite, buckets = a few thousand), so by the pigeonhole principle collisions must happen. The question is not *whether* they happen but *how* the table handles them." },
      { type: "buckets", capacity: 6, buckets: [
        null, [{ key: "cat" }, { key: "dog" }], null, null, null, null,
      ], collisionIndex: 1, caption: "Both 'cat' and 'dog' land in bucket 1 — a collision." },
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      { type: "buckets", capacity: 6, buckets: [
        null,
        [{ key: "cat", value: 1 }, { key: "dog", value: 2 }, { key: "bat", value: 3 }],
        null,
        [{ key: "ant", value: 4 }],
        null,
        null,
      ], caption: "Bucket 1 chains three colliding keys; bucket 3 has one." },
      { type: "code", title: "insert", code:
`b = buckets[hash(k) % m]
...
b.append((k, v))` },
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "In open addressing, every entry lives inside the array. A collision on slot i means try i+1, then i+2 (linear), or i+1², i+2² (quadratic), or some second-hash step (double hashing)." },
      { type: "callout", kind: "perf", title: "Why CPython uses it",
  ...
  },
  {
    ...
    sections: [
      { type: "buckets", capacity: 8, buckets: [
        ...
        null, null, null, null,
      ], probeIndices: [2, 3, 4], caption: "'d' hashed to 2, found it full → probed 3 → still full → landed at 4." },
      { type: "code", code:
`def insert(k, v):
    i = hash(k) % m
    while table[i] is not None and table[i][0] != k:
        i = (i + 1) % m
    table[i] = (k, v)` },
      { type: "callout", kind: "warn", title: "Primary clustering",
...
`def probe(i, k):
    for j in range(m):
      ...
      { type: "callout", kind: "info", title: "Table size matters",
...
`def probe(k):
    ...
    for _ in range(m):
        yield i
        i = (i + step) % m` },
      { type: "callout", kind: "did", title: "Near-uniform distribution",
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "During insertion, if the current entry has probed less than the incoming key, swap them and continue with the displaced entry. Keeps the variance of probe distances tiny." },
    ],
  },
  {
    slug: "cuckoo-hashing",
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Every key is stored in one of two positions, given by h1 and h2. On collision, the incumbent is kicked out into its alternate slot — cuckoo-style. Look-up checks only two slots, guaranteeing O(1) worst case." },
    ],
  },
  {
    slug: "rehashing",
...
`def rehash(table, new_cap):
    ...
    return new_table` },
      { type: "callout", kind: "perf", title: "Amortised analysis",
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Common policy: double when α > 0.66; some implementations halve when α < 0.16. CPython grows by factor 4× for small dicts, 2× thereafter, to reduce the number of rehashes." },
    ],
  },
  {
    ...
    sections: [
      { type: "practice", groups: [
        { level: "Beginner", items: [
          { title: "LC 1 · Two Sum", url: "https://leetcode.com/problems/two-sum/", difficulty: "Easy", pattern: "hash-map complement" },
          { title: "LC 217 · Contains Duplicate", url: "https://leetcode.com/problems/contains-duplicate/", difficulty: "Easy", pattern: "set membership" },
        ]},
        { level: "Intermediate", items: [
          { title: "LC 49 · Group Anagrams", url: "https://leetcode.com/problems/group-anagrams/", difficulty: "Medium", pattern: "hash-map bucket" },
          { title: "LC 560 · Subarray Sum Equals K", url: "https://leetcode.com/problems/subarray-sum-equals-k/", difficulty: "Medium", pattern: "prefix sum + hash" },
          { title: "LC 128 · Longest Consecutive Sequence", url: "https://leetcode.com/problems/longest-consecutive-sequence/", difficulty: "Medium", pattern: "hash set" },
        ]},
        { level: "Advanced", items: [
          { title: "LC 187 · Repeated DNA Sequences", url: "https://leetcode.com/problems/repeated-dna-sequences/", difficulty: "Medium", pattern: "rolling hash" },
          { title: "LC 76 · Minimum Window Substring", url: "https://leetcode.com/problems/minimum-window-substring/", difficulty: "Hard", pattern: "sliding window + hash" },
        ]},
      ]},
    ],
  },
    ...
    sections: [
      { type: "quiz", items: [
        { q: "Which is NOT a property of a good hash function?",
          ...
          answer: 2,
          explain: "Reversibility is a property of *cryptographic* hashes' failure mode, not a design goal — good hashes are one-way in practice." },
  ...
  },
  {
    ...
    sections: [
      { type: "references", items: [
        { label: "PEP 456 — Secure and interchangeable hash algorithm", url: "https://peps.python.org/pep-0456/" },
        { label: "CPython Objects/dictobject.c", url: "https://github.com/python/cpython/blob/main/Objects/dictobject.c" },
        { label: "CLRS · Chapter 11 · Hash Tables", url: "https://mitpress.mit.edu/9780262046305/introduction-to-algorithms/" },
        { label: "Sedgewick — Universal Hashing lecture", url: "https://algs4.cs.princeton.edu/34hash/" },
      ]},
    ],
  },
];
