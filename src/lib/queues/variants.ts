...
export const QUEUE_VARIANTS: QueueLesson[] = [
  {
    ...
    eyebrow: "Variants · 1",
    description:
      "The textbook FIFO — array-backed with front and rear pointers that only move right.",
    difficulty: "Beginner",
    readMinutes: 6,
    sections: [
      {
        type: "theory",
        text: "A linear queue uses a fixed-size array with two indices. Enqueue writes to buf[rear] and advances rear; dequeue reads buf[front] and advances front. Simple — and wasteful once the front leaves slots behind.",
      },
      {
        type: "viz",
        items: [10, 20, 30, 40],
        caption: "Live window inside a 6-slot buffer.",
        capacity: 6,
      },
      {
        type: "code",
        title: "class LinearQueue",
  ...
  },
  {
    ...
    eyebrow: "Variants · 2",
    description:
      "Wrap the rear back to slot 0 to reclaim empty space — a fixed-capacity queue that never wastes slots.",
    difficulty: "Intermediate",
    readMinutes: 8,
    sections: [
      {
        type: "theory",
        text: "Treat the buffer as a ring. Both front and rear advance modulo the capacity, so when rear reaches the end it wraps to index 0 and reuses the space dequeue vacated.",
      },
      {
        type: "viz",
        items: [30, 40, 10, 20],
  ...
  },
  {
    ...
    eyebrow: "Variants · 3",
    description:
      "O(1) push and pop at both ends — the Swiss Army knife of sliding-window and monotonic algorithms.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      {
        type: "theory",
        text: "A deque generalises the queue: you can enqueue or dequeue at either end in O(1). Python ships one in `collections.deque`, implemented as a doubly linked list of 64-slot blocks.",
      },
      {
        type: "code",
...
dq.pop()             # 4             — dequeue rear`,
  ...
  },
  {
    ...
    eyebrow: "Variants · 4",
    description:
      "Not FIFO — the smallest (or largest) priority leaves first. Backed by a binary heap.",
    difficulty: "Intermediate",
    readMinutes: 7,
    sections: [
      {
        type: "theory",
        text: "A priority queue orders items by an external key, not by arrival time. Python's `heapq` module maintains a binary min-heap on top of a list — push and pop are O(log n) and peek is O(1).",
      },
      {
        type: "code",
        code: `import heapq
...
heapq.heappop(pq)   # (1, 'task A') — smallest priority first`,
  ...
  },
  {
    ...
    eyebrow: "Variants · 5",
    description:
      "No fixed capacity, guaranteed O(1) — a linked list with head and tail pointers is the classic textbook queue.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      {
        type: "theory",
        text: "Keep two pointers: head (the front) and tail (the rear). Dequeue unlinks head; enqueue appends to tail. No resizing, no wrap-around, no wasted slots — but every node costs one pointer of overhead.",
      },
      {
        type: "code",
        title: "class LinkedListQueue",
        code: `class Node:
...
class LinkedListQueue:
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Use two stacks: `inbox` for incoming items and `outbox` for outgoing. Enqueue just pushes onto inbox. Dequeue: if outbox is empty, drain inbox into outbox (which reverses the order), then pop outbox. Each element moves at most twice — amortised O(1).",
      },
      {
        type: "code",
        title: "class QueueFromStacks",
