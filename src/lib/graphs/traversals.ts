...
export const G_TRAVERSALS: GLesson[] = [
  {
    ...
    description:
      "Two ways to visit every reachable vertex — and why almost every graph algorithm is one of them in disguise.",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      {
      ...
      },
      {
        type: "graphViz",
        spec: DEMO,
        caption: "We'll traverse this graph starting from A in every lesson below.",
      },
      {
        type: "table",
        headers: ["", "BFS", "DFS"],
...

  {
    ...
    description:
      "Explore level by level with a queue — the shortest-path algorithm for unweighted graphs.",
    difficulty: "Beginner",
    readMinutes: 7,
    sections: [
      {
        type: "theory",
        text: "BFS starts at a source vertex, visits all of its direct neighbours, then all of their unvisited neighbours, and so on — expanding outward one full 'ring' at a time. Because it explores in order of distance, the first time BFS reaches a vertex is guaranteed to be via a shortest path (in terms of edge count) from the source.",
      },
      {
        type: "bfsPlayer",
        spec: DEMO,
        start: "A",
        caption: "Watch the queue grow and drain — each dequeue is a visit.",
      },
      {
        type: "code",
...
print(bfs(graph, "A"))   # ['A', 'B', 'C', 'D', 'E', 'F']`,
      ...
      {
        type: "code",
        title: "python — shortest path + distances",
        code: `def bfs_distances(graph, start):
    dist = {start: 0}
    queue = deque([start])
    while queue:
        u = queue.popleft()
    ...
    return dist   # edge-count distance from start to every reachable vertex`,
      ...
      {
        type: "dryRun",
        headers: ["Step", "Dequeue", "Queue after", "Visited"],
        rows: [
          ["1", "—", "[A]", "{A}"],
          ["2", "A", "[B, C]", "{A, B, C}"],
          ["3", "B", "[C, D]", "{A, B, C, D}"],
        ...
        caption:
          "D is enqueued twice as a candidate (from B and from C) but only visited once — the `visited` check on enqueue prevents duplicates.",
      },
      {
        type: "complexity",
        rows: [
      ...
      },
      {
        ...
        text: "'Shortest path' + 'unweighted' + 'grid or graph' is the classic BFS tell. If you see weights, that's your cue to reach for Dijkstra instead.",
      },
      {
        type: "practice",
        groups: [
...
print(dfs_recursive(graph, "A"))   # ['A', 'B', 'D', 'C', 'E', 'F']`,
      ...
      },
      {
        type: "callout",
        kind: "tip",
        text: "sys.setrecursionlimit() is a band-aid, not a fix — for graphs where depth isn't bounded, prefer the iterative DFS in the next lesson.",
      },
    ],
  },

  {
    ...
    sections: [
      {
        type: "theory",
        text: "The iterative version replaces Python's call stack with an explicit list used as a stack. It avoids recursion-limit issues entirely and makes the 'push / pop' mechanics of DFS visible instead of implicit — which is also why interviewers sometimes specifically ask for it.",
      },
      {
        type: "code",
        title: "python — full implementation",
...
print(dfs_iterative(graph, "A"))   # ['A', 'B', 'D', 'C', 'E', 'F']`,
      ...
      },
      {
        ...
        items: [
          {
            q: "Why does the iterative DFS check `if u in visited: continue` after popping, rather than only checking before pushing?",
            choices: [
              "It doesn't matter, it's just a style choice",
              "A vertex can be pushed onto the stack multiple times before it's first processed",
              "It makes the algorithm faster",
              "It's required to avoid a syntax error",
            ],
            answer: 1,
            explain:
              "Two different neighbours can both push the same unvisited vertex before either is popped, so the stack can contain duplicates — the post-pop check discards the stale ones.",
          },
        ],
      },
    ],
...

  {
    ...
    sections: [
      {
        type: "theory",
        text: "A connected component is a maximal set of vertices where every pair is joined by some path. Finding all components is a direct application of traversal: run BFS or DFS from any unvisited vertex, mark everything it reaches as one component, then repeat from the next unvisited vertex.",
      },
      {
        type: "graphViz",
        spec: DISCONNECTED,
        caption: "Three components: {A, B, C}, {D, E}, and the isolated vertex F.",
      },
      {
        type: "code",
        title: "python",
        code: `def connected_components(graph, vertices):
    visited = set()
    components = []
...
# [['A', 'B', 'C'], ['D', 'E'], ['F']]`,
        ...
        rows: [
          {
            op: "Time",
            time: "O(V + E)",
            note: "every vertex and edge is visited exactly once overall",
          },
          { op: "Space", time: "O(V)" },
        ],
      },
      {
        type: "callout",
        kind: "did",
        text: "For directed graphs, this same idea generalizes into strongly connected components — but a directed edge only lets you traverse one way, so it needs a smarter algorithm (Kosaraju's or Tarjan's), covered later in Algorithms.",
      },
      {
        type: "references",
        items: [
