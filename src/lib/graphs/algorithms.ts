...
export const G_ALGORITHMS: GLesson[] = [
  {
    ...
    sections: [
      ...
      { type: "callout", kind: "info", text: "Every algorithm below is BFS or DFS with one extra piece of bookkeeping — a distance table, a priority choice, or a disjoint-set structure." },
    ],
  },
...
print(dijkstra(graph, "S"))   # {'S': 0, 'A': 2, 'B': 3, 'C': 6, 'T': 6}` },
      { type: "dryRun", headers: ["Locked in", "dist[S]", "dist[A]", "dist[B]", "dist[C]", "dist[T]"], rows: [
        ["S", "0", "∞", "∞", "∞", "∞"],
        ["A (2)", "0", "2", "5", "6", "∞"],
        ["B (3)", "0", "2", "3", "6", "6"],
        ["C (6)", "0", "2", "3", "6", "6"],
      ...
      { type: "complexity", rows: [
        ...
        { op: "Array (no heap)", time: "O(V²)", space: "O(V)", note: "simpler, fine for dense/small graphs" },
      ]},
      { type: "mistakes", items: [
      ...
      { type: "callout", kind: "interview", title: "Interview signal", text: "'Cheapest flight route', 'minimum cost path', 'network delay time' with non-negative weights — that's Dijkstra." },
      { type: "practice", groups: [
        { level: "Intermediate", items: [
          { title: "Network Delay Time", url: "https://leetcode.com/problems/network-delay-time/", difficulty: "Medium", pattern: "Dijkstra" },
        ] },
        { level: "Advanced", items: [
          { title: "Path with Maximum Probability", url: "https://leetcode.com/problems/path-with-maximum-probability/", difficulty: "Medium", pattern: "Dijkstra variant (max product)" },
          { title: "Cheapest Flights Within K Stops", url: "https://leetcode.com/problems/cheapest-flights-within-k-stops/", difficulty: "Medium", pattern: "Bellman-Ford / constrained Dijkstra" },
        ] },
      ] },
      { type: "references", items: [
        { label: "CLRS §24.3 — Dijkstra's algorithm", url: "https://mitpress.mit.edu/9780262046305/" },
      ] },
    ],
  },

  {
    ...
    description: "Single-source shortest paths that also handles negative weights — and detects negative cycles.",
    difficulty: "Intermediate",
    readMinutes: 7,
    sections: [
      { type: "theory", text: "Bellman-Ford relaxes every edge in the graph, V−1 times. That's it. Where Dijkstra commits greedily to the closest vertex, Bellman-Ford is deliberately brute-force — by trying every edge repeatedly, it correctly propagates negative weights, and a final V-th pass that still finds an improvement proves a negative cycle exists." },
      { type: "bellmanFordPlayer", spec: WEIGHTED_WITH_NEGATIVE, start: "S", caption: "Note the negative A→B edge — Dijkstra would get this wrong." },
...
# ({'S': 0, 'A': 4, 'B': 1, 'T': 3}, False)` },
      ...
      { type: "callout", kind: "interview", title: "Interview signal", text: "Any 'shortest path' problem that explicitly mentions negative weights, or asks you to detect an arbitrage/negative cycle, wants Bellman-Ford." },
      { type: "references", items: [
        { label: "CLRS §24.1 — The Bellman-Ford algorithm", url: "https://mitpress.mit.edu/9780262046305/" },
      ] },
    ],
  },

  {
    ...
    description: "Grow a minimum spanning tree one cheapest edge at a time, from a single starting vertex.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      { type: "theory", text: "A minimum spanning tree (MST) connects all vertices with the minimum total edge weight, using exactly V−1 edges and no cycles. Prim's algorithm builds it greedily: start from any vertex, and repeatedly add the cheapest edge that connects a vertex already in the tree to one that isn't." },
      { type: "primPlayer", spec: MST_DEMO, start: "A", caption: "Each step adds the cheapest edge crossing the tree's current boundary." },
      { type: "code", title: "python — full implementation (binary heap)", code:
`import heapq
...
def prim(graph, start):
    ...
    return mst, cost` },
      { type: "complexity", rows: [
        { op: "Binary heap", time: "O(E log V)", space: "O(V + E)" },
      ]},
      { type: "mistakes", items: [
      ...
      { type: "callout", kind: "tip", text: "Prim is usually faster when the graph is dense (many edges relative to vertices); Kruskal tends to win on sparse graphs since it doesn't need per-vertex adjacency exploration." },
    ],
  },

  {
    ...
    description: "Sort every edge by weight, then greedily add any edge that doesn't create a cycle.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      { type: "theory", text: "Kruskal's algorithm takes a completely different angle on the same problem: sort every edge in the graph by weight, then walk the sorted list adding an edge whenever its two endpoints aren't already connected. Union-Find is what makes 'already connected?' an almost-O(1) question instead of a full traversal." },
      { type: "kruskalPlayer", spec: MST_DEMO, caption: "Edges are considered cheapest-first; a red 'skip' means it would form a cycle." },
      { type: "code", title: "python — full implementation", code:
`class DSU:
...
def kruskal(vertices, edges):
    ...
    return mst, cost` },
      { type: "complexity", rows: [
        ...
        { op: "Union-Find operations", time: "O(E · α(V))", note: "α is the inverse Ackermann function — effectively constant" },
      ...
      { type: "callout", kind: "interview", title: "Interview signal", text: "'Connect all cities with minimum cable cost' or 'minimum cost to connect all points' — both are MST problems; Kruskal is usually the faster one to code correctly under time pressure." },
      { type: "practice", groups: [
        { level: "Intermediate", items: [
          { title: "Min Cost to Connect All Points", url: "https://leetcode.com/problems/min-cost-to-connect-all-points/", difficulty: "Medium", pattern: "MST (Prim or Kruskal)" },
        ] },
      ] },
    ],
  },

  {
    ...
    description: "A valid processing order for a DAG — every dependency comes before what depends on it.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      { type: "theory", text: "A topological sort orders the vertices of a directed acyclic graph (DAG) so that every edge u→v places u before v. It only exists for DAGs — a cycle makes it impossible to satisfy every ordering constraint at once. Kahn's algorithm builds the order using in-degrees: repeatedly remove any vertex with in-degree 0 (nothing left depends on going through it first)." },
      { type: "topoSortPlayer", spec: DAG, caption: "'shirt' before 'jacket'; 'pants' before both 'shoes' and 'belt' — a getting-dressed dependency graph." },
      { type: "code", title: "python — Kahn's algorithm (BFS-based)", code:
...
# ['shirt', 'pants', 'jacket', 'shoes', 'belt']  (one of several valid orders)` },
      { type: "code", title: "python — DFS-based alternative", code:
`def topological_sort_dfs(graph, vertices):
    ...
    return order[::-1]           # reverse post-order = topological order` },
      ...
      { type: "callout", kind: "interview", title: "Interview signal", text: "'Course schedule', 'build order', 'task dependencies' — any 'X must happen before Y' phrasing is a topological sort problem." },
      { type: "practice", groups: [
        { level: "Intermediate", items: [
          { title: "Course Schedule", url: "https://leetcode.com/problems/course-schedule/", difficulty: "Medium", pattern: "Cycle detection + topo sort" },
          { title: "Course Schedule II", url: "https://leetcode.com/problems/course-schedule-ii/", difficulty: "Medium", pattern: "Topological sort" },
        ] },
      ] },
    ],
  },
...
print(dsu.find(0) == dsu.find(3))   # False` },
      { type: "complexity", rows: [
        { op: "find / union (with both optimizations)", time: "O(α(V))", note: "amortized, effectively O(1)" },
        { op: "find / union (no optimizations)", time: "O(V)", note: "worst case — a degenerate chain" },
      ]},
      { type: "mistakes", items: [
      ...
      { type: "callout", kind: "interview", title: "Interview signal", text: "'Number of connected components', 'redundant connection', 'accounts merge' — any problem about grouping/merging things over a stream of pair-relations wants Union-Find." },
      { type: "practice", groups: [
        { level: "Intermediate", items: [
          { title: "Number of Provinces", url: "https://leetcode.com/problems/number-of-provinces/", difficulty: "Medium", pattern: "Union-Find" },
          { title: "Redundant Connection", url: "https://leetcode.com/problems/redundant-connection/", difficulty: "Medium", pattern: "Union-Find cycle detection" },
        ] },
        { level: "Advanced", items: [
          { title: "Accounts Merge", url: "https://leetcode.com/problems/accounts-merge/", difficulty: "Medium", pattern: "Union-Find over strings" },
        ] },
      ] },
    ],
  },

  {
    ...
    readMinutes: 4,
    sections: [
      { type: "table", headers: ["If you need…", "Reach for"], rows: [
      ...
      { type: "quiz", items: [
        { q: "A graph has a negative-weight edge. Which shortest-path algorithm is safe to use?", choices: ["Dijkstra", "Bellman-Ford", "Either — they always agree", "Neither works with negative weights"], answer: 1, explain: "Dijkstra's greedy 'lock in the closest vertex' step assumes distances only ever increase — a later negative edge can violate that after the fact." },
        { q: "Which data structure makes Kruskal's cycle check fast?", choices: ["A priority queue", "A hash set of visited vertices", "Union-Find", "An adjacency matrix"], answer: 2, explain: "Union-Find answers 'are u and v already connected?' in near-constant time as edges are added." },
      ]},
      { type: "callout", kind: "info", title: "What's next", text: "This covers the core algorithm set requested for this pass. Floyd-Warshall, A*, strongly connected components (Kosaraju/Tarjan), and network-flow algorithms (Ford-Fulkerson, Edmonds-Karp) are natural next additions to this tier." },
    ],
  },
];
