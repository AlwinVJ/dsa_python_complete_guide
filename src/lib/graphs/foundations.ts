...
export const G_FOUNDATIONS: GLesson[] = [
  {
    ...
    description: "A graph is a set of vertices connected by edges — the most general data structure in computer science.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      { type: "theory", text: "A graph G = (V, E) is a pair of a set of vertices V and a set of edges E, where each edge connects two vertices. Unlike trees, graphs allow cycles, multiple connected components, and edges in any direction. Almost every relationship you can imagine — friendships, roads, dependencies, molecules — is naturally a graph." },
      { type: "graphViz", spec: SIMPLE, caption: "A tiny undirected graph with 5 vertices and 5 edges." },
      { type: "callout", kind: "info", title: "Where we're headed", text: "Foundations → Graph Types → Representations → Traversals → Algorithms → Review. Master each tier in order." },
      { type: "code", title: "python", code: `# The most common Python representation: adjacency list.\nG = {\n    "A": ["B", "C"],\n    "B": ["A", "D"],\n    "C": ["A", "D"],\n    "D": ["B", "C", "E"],\n    "E": ["D"],\n}` },
...

  {
    ...
    description: "Where trees and lists fall short, and how a graph shape solves the general problem.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      { type: "theory", bullets: [
      ...
      { type: "callout", kind: "did", title: "One structure to rule them all", text: "Every data structure you've studied so far — arrays, lists, trees, tries — is a special case of a graph. Master graphs and you have a mental model for the rest." },
    ],
  },

  {
    ...
    readMinutes: 4,
    sections: [
      { type: "table", headers: ["Problem", "Graph flavour", "Algorithm"], rows: [
...

  {
    ...
    readMinutes: 6,
    sections: [
      { type: "theory", text: "Graph algorithms lean heavily on precise vocabulary. Skim this once, then refer back as you learn new algorithms." },
      ...
      { type: "graphViz", spec: DISCONNECTED, caption: "Two connected components: {A,B,C} and {D,E} — plus isolated vertex F." },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "A vertex represents an entity — a city, a user, a task, a state. Vertices carry an identity and may carry arbitrary attributes (colour, label, coordinates, cost)." },
      { type: "code", title: "python", code: `# Vertices as any hashable key: strings, ints, tuples.\nV = {"A", "B", "C"}                # a set of vertex IDs\ncoords = {"A": (0, 0), "B": (2, 3)} # per-vertex attributes` },
      { type: "callout", kind: "tip", text: "Pick a stable vertex ID type early — mixing strings and ints in one graph is a common bug." },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "An edge (u, v) says 'u is related to v'. Edges can carry weights, capacities, timestamps, or types. In an undirected graph (u, v) and (v, u) mean the same thing; in a directed graph they differ." },
      { type: "graphViz", spec: DIRECTED, caption: "A directed graph — the arrow shows edge orientation." },
      { type: "code", title: "python", code: `# An edge is typically a tuple (or a dict for richer data).\nedges = [\n    ("A", "B", 3),   # weighted\n    ("B", "C", 1),\n    ("C", "A", 2),\n]` },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Vertex v is adjacent to u if the edge (u, v) exists. The neighbourhood N(u) is the set of all such v. Every traversal boils down to 'from the current vertex, iterate its neighbours'." },
      { type: "code", title: "python", code: `def neighbours(G, u):\n    """Return N(u) for an adjacency-list graph."""\n    return G.get(u, [])` },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "A path is a sequence v₀ → v₁ → … → vₖ where each consecutive pair is an edge. Path length = number of edges (or the sum of weights in a weighted graph)." },
      { type: "graphViz",
...

  {
    ...
    readMinutes: 2,
    sections: [
      { type: "theory", text: "A simple path repeats no vertex. Dijkstra, BFS, and Bellman-Ford all return simple paths — a shortest path is never worse for being simple." },
      { type: "callout", kind: "info", title: "Why the distinction matters", text: "'Number of walks of length k' has closed-form answers (matrix powers); 'number of simple paths' is NP-hard in the general case." },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "A cycle is a path that ends where it started. In a DAG (directed acyclic graph) cycles are forbidden. In dependency systems, cycles mean 'circular dependency' — always a bug." },
...

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "An undirected graph is connected if a path exists between every pair of vertices. If not, it splits into components. A directed graph is strongly connected if every pair is mutually reachable; weakly connected if that holds after ignoring edge directions." },
      { type: "graphViz", spec: SIMPLE, caption: "One connected component covering all 5 vertices." },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "A disconnected graph has multiple components. BFS or DFS from a single source visits only its component — to visit every vertex you must loop across all vertices and restart traversal on any unvisited one." },
      { type: "graphViz", spec: DISCONNECTED, caption: "Three components: {A,B,C}, {D,E}, {F}." },
      { type: "code", title: "python", code: `def visit_all(G):\n    seen = set()\n    for u in G:\n        if u not in seen:\n            dfs(u, G, seen)   # start a fresh traversal per component` },
    ],
  },

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "Degree(u) counts edges incident on u. The handshake lemma says Σ deg(u) = 2|E| in an undirected graph — every edge contributes to two vertices." },
      { type: "code", title: "python", code: `deg = {u: len(G[u]) for u in G}` },
      { type: "table", headers: ["Vertex", "Neighbours", "Degree"], rows: [
        ["A", "B, C", "2"],
        ["B", "A, D", "2"],
        ["C", "A, D", "2"],
        ["D", "B, C, E", "3"],
...

  {
    ...
    readMinutes: 3,
    sections: [
      { type: "theory", text: "In a directed graph, in-degree(u) counts edges (·, u). Vertices with in-degree 0 are 'sources' — safe to schedule first. Kahn's topological sort starts from every zero-in-degree vertex." },
      { type: "code", title: "python", code: `from collections import defaultdict\nindeg = defaultdict(int)\nfor u in G:\n    for v in G[u]:\n        indeg[v] += 1` },
    ],
  },

  {
    ...
    readMinutes: 2,
    sections: [
      { type: "theory", text: "Out-degree(u) counts edges (u, ·). In a random-walk model, out-degree determines transition options." },
...

  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "An unweighted edge is a plain connection — its 'cost' is 1. A weighted edge carries a numeric label — distance, latency, capacity, price. The algorithms you pick depend on this distinction." },
      { type: "graphViz", spec: WEIGHTED, caption: "Weighted directed graph — labels are edge costs." },
      { type: "table", headers: ["Shortest path in…", "Algorithm"], rows: [
...

  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Density = |E| / max|E|. In a sparse graph E ≈ V; in a dense graph E ≈ V². Sparse graphs prefer adjacency lists (O(V+E) space); dense graphs prefer adjacency matrices (O(V²) space, O(1) edge lookup)." },
      { type: "table", headers: ["Density", "|E| range", "Best representation"], rows: [
...

  {
    ...
    sections: [
      ...
      { type: "callout", kind: "interview", text: "Interviewers love asking 'is this graph bipartite?' — 2-colour with BFS/DFS in O(V + E)." },
    ],
  },

  {
    ...
    readMinutes: 5,
    sections: [
      { type: "theory", text: "Understanding the memory shape of a graph explains why Dijkstra runs faster on adjacency lists and why edge-list traversals are cache-friendly for Bellman-Ford." },
      { type: "memoryDiagram", rows: [
        { label: "Adj list", value: "{A: [B, C], B: [A, D], C: [A, D], D: [B, C, E], E: [D]}", note: "O(V + E) space" },
        { label: "Adj matrix", value: "5×5 array of 0/1", note: "O(V²) space" },
        { label: "Edge list", value: "[(A,B), (A,C), (B,D), (C,D), (D,E)]", note: "O(E) space" },
      ]},
      { type: "callout", kind: "perf", text: "CPUs love contiguous arrays. Adjacency-list neighbourhoods in Python are already Python lists (contiguous pointers) — good enough for most interview problems." },
    ],
  },

  {
    ...
    sections: [
      { type: "table", headers: ["Advantages", "Disadvantages"], rows: [
        ...
        ["Enables shortest paths, flows, MSTs", "Traversal is O(V + E) — expensive on huge graphs"],
        ["Handles multiple components", "Correct representation matters for performance"],
        ["Directly supports weights and directions", "Cycles complicate recursion (recursion limits)"],
      ]},
    ],
  },

  {
    ...
    sections: [
      ...
      { type: "callout", kind: "info", title: "Next up", text: "Explore Graph Types to see how each variant restricts or extends this base model, then Representations to lock in how the data lives in memory." },
    ],
  },
];
