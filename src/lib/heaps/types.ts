...
export type HComplex  = { type: "complexity"; rows: { op: string; time: string; space?: string; note?: string }[] };
export type HMistakes = { type: "mistakes"; items: string[] };
export type HTip      = { type: "tip"; text: string; title?: string };
export type HCallout  = { type: "callout"; kind: "info" | "warn" | "perf" | "did" | "tip" | "interview"; title?: string; text: string };
export type HQuizItem = { q: string; choices: string[]; answer: number; explain?: string };
export type HQuiz     = { type: "quiz"; items: HQuizItem[] };
export type HPractice = { type: "practice"; groups: { level: "Beginner" | "Intermediate" | "Advanced"; items: { title: string; url: string; difficulty: "Easy" | "Medium" | "Hard"; pattern?: string; time?: string }[] }[] };
...
export type HSection =
  | HeapVizSec | HeapPlaygroundSec | IndexDiagramSec | TreeVizSec
  | HTheory | HCode | HDryRun | HComplex | HMistakes | HTip | HCallout
  | HQuiz | HPractice | HRefs | HInterview | HHeading | HTable | HFAQ;
