...
export const H_FOUNDATIONS: HLesson[] = [
  {
    ...
    description:
      "A heap is a complete binary tree that always keeps the extreme value at the root — the shape behind priority queues, Dijkstra, and heapq.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "A heap is a complete binary tree that obeys a heap property: every parent compares favourably against its children. In a min-heap the smallest value sits at the root; in a max-heap the largest. Heaps are the tree behind the priority queue — the abstract data type that answers 'what should I look at next?' in O(log n).",
      },
      {
        type: "heapViz",
        data: [1, 3, 5, 7, 9, 8],
      ...
      {
        type: "code",
        title: "Python heapq — the built-in",
        code: `import heapq
...
h[0]               # 3  — peek in O(1)`,
  ...
  },
  {
    ...
    description:
      "The one operation heaps are unbeatable at: give me the next extreme value, over and over, in O(log n).",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "table",
        headers: ["Structure", "Insert", "Peek min/max", "Extract min/max"],
        rows: [
      ...
      {
        type: "theory",
        text: "Heaps trade full ordering for partial ordering. You cannot list values in sorted order without repeatedly extracting — but if you only ever need the next best item, a heap beats every alternative.",
      },
      {
        type: "callout",
        kind: "did",
        title: "The unifying idea",
  ...
  },
  {
    ...
    description:
      "Root, leaf, height, complete tree, heap property — the vocabulary every later lesson assumes.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        ...
        kind: "min",
        caption:
          "A complete binary tree stored in an array. Root = index 0; leaves live in the lower half of the array.",
      },
      {
        type: "table",
        ...
        rows: [
          ...
          [
            "Heap property",
            "Parent-child ordering. Min-heap: parent ≤ children. Max-heap: parent ≥ children.",
          ],
  ...
  },
  {
    ...
    description:
      "Why heaps insist on the shape of a complete binary tree — and what breaks if that shape is violated.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "A complete binary tree fills every level top-to-bottom, left-to-right. Two properties fall out for free:",
      },
      {
        type: "theory",
        bullets: [
      ...
      },
      {
        ...
        kind: "min",
        caption:
          "Complete: nine nodes fill three levels perfectly, then the fourth level fills from the left.",
      },
      {
        type: "callout",
        kind: "warn",
        title: "Non-complete = broken index math",
  ...
  },
  {
    ...
    description:
      "The single ordering rule that defines a heap — and how min vs max heaps differ from BSTs.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "The heap property is a local rule: it only relates a node to its immediate children, not siblings or cousins. That local rule is enough to guarantee the root is globally extreme.",
      },
      {
        type: "table",
        headers: ["Structure", "Ordering rule", "Root holds"],
        rows: [
      ...
      },
      {
        ...
        kind: "min",
        caption:
          "Valid min-heap: parents (2, 5, 3) are all ≤ their children. Siblings 5 and 3 are out of order — that's fine.",
      },
    ],
  },
  {
    ...
    description:
      "Three arithmetic formulas replace pointer chasing — this is why heaps live in arrays.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
...
right(i)  = 2 * i + 2`,
      ...
      {
        type: "indexDiagram",
        data: [10, 20, 15, 30, 40, 50, 100, 25],
        focus: 1,
        caption: "For index 1 (value 20): parent is 0, left child is 3, right child is 4.",
      },
      {
        type: "theory",
        text: "Because indices are just integers, moving 'up' the tree is a single arithmetic shift. That eliminates the pointer overhead of a linked tree and keeps every heap operation cache-friendly.",
      },
      {
        type: "callout",
        kind: "did",
        title: "Zero-indexed vs one-indexed",
  ...
  },
  {
    ...
    description:
      "Level-order traversal of the tree IS the array. That single fact powers everything else.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "Read the tree level by level, left to right, and write down each value. The resulting sequence is exactly the array representation. Reading it back the same way reconstructs the tree.",
      },
      {
        type: "heapViz",
        data: [1, 3, 5, 7, 9, 8, 6],
      ...
      {
        type: "code",
        title: "Python — build tree from array (mental model only)",
        code: `# You NEVER actually build these nodes for a heap — the array IS the tree.
class Node:
    __slots__ = ("v", "l", "r")
...
def tree_from_heap(arr, i=0):
    ...
    return n`,
      },
      {
        type: "callout",
        kind: "perf",
        title: "Why array beats pointers",
  ...
  },
  {
    ...
    description:
      "How Python actually lays out a heap in memory — and why it is O(1) space overhead per element.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "Python's `heapq` stores the heap in a regular `list`. A list is a contiguous array of PyObject pointers, so a heap of n integers costs exactly the same as a plain list of n integers — no per-node header, no left/right pointers, no null slots.",
      },
      {
        type: "table",
        headers: ["Layout", "Bytes per element", "Overhead"],
        rows: [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        text: "Always test `if left < n` before dereferencing a child. Off-by-one here is the #1 heap bug.",
      },
    ],
  },
  {
    ...
    description:
      "How the underlying array grows during push and shrinks after pop — and why it stays amortized cheap.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "A heap of n elements needs a container that supports append and pop-back in amortized O(1). Python's `list` gives exactly that: it over-allocates on growth so most appends cost O(1), with occasional O(n) copies when the capacity doubles.",
      },
      {
        type: "table",
        headers: ["Operation", "Container work", "Heap work", "Total"],
        rows: [
      ...
      },
      {
        type: "callout",
        kind: "warn",
        title: "Do not preserve iteration order",
  ...
  },
  {
    ...
    description:
      "All heap operations at a glance — what the numbers are, and where they come from.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        ...
        rows: [
          ...
          { op: "heap sort", time: "O(n log n)", space: "O(1)", note: "Heapify + n extracts." },
          {
            ...
            time: "O(n)",
            space: "O(1)",
            note: "Heap doesn't help search.",
          },
          {
            op: "delete arbitrary",
            time: "O(n)",
            space: "O(1)",
            note: "Search + swap-last + sift both ways.",
          },
        ],
      },
      {
        type: "callout",
        kind: "perf",
        title: "Why heapify is O(n), not O(n log n)",
    ...
    sections: [
      {
        ...
        rows: [
          { op: "storage", time: "—", space: "O(n)", note: "One slot per element in the array." },
          {
            ...
            time: "—",
            space: "O(log n)",
            note: "Iterative versions use O(1).",
          },
          {
            op: "heap sort in place",
            time: "—",
            space: "O(1)",
            note: "Sorts inside the input list.",
          },
      ...
      {
        type: "tip",
        text: "Prefer iterative sift routines when writing your own heap — the extra stack frame from recursion is real, and `sys.setrecursionlimit` will bite you on very deep heaps.",
      },
    ],
  },
];
