...
export const H_MIN_HEAP: HLesson[] = [
  {
    ...
    eyebrow: "Min Heap · 1",
    description: "The default flavour: the smallest value lives at the root and is available in O(1).",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      { type: "theory", text: "A min-heap is a complete binary tree where every parent is ≤ each of its children. That single rule guarantees the smallest value in the whole structure sits at the root, ready to be read in constant time or removed in logarithmic time." },
      { type: "heapViz", data: [1, 3, 5, 7, 9, 8, 6], kind: "min",
        caption: "Root 1 is the minimum. Every downward edge points to a value ≥ its parent." },
      { type: "callout", kind: "info", title: "Python default",
  ...
  },
  {
    ...
    title: "Structure",
    eyebrow: "Min Heap · 2",
    description: "Complete tree + parent ≤ children — the two invariants every operation must preserve.",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      { type: "theory", bullets: [
  ...
  },
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Every heap operation follows the same pattern: temporarily break the order invariant at one node, then swap that node up or down until order is restored. The shape invariant is preserved by always adding and removing at the last index." },
      { type: "table", headers: ["Op", "Where it starts", "Direction of fix"], rows: [
  ...
  },
  {
    ...
    readMinutes: 6,
    sections: [
      { type: "code", title: "min_heap.py", code:
`class MinHeap:
    ...
    def _sift_down(self, i):
        ...
        while True:
            ...
            i = smallest`,
        explanation: "The two invariants only ever break at one node, so both sift routines are single-path — they run in O(height) = O(log n)." },
    ],
  },
  {
    slug: "insert",
    ...
    readMinutes: 5,
    sections: [
      { type: "theory", text: "Insertion works in two phases: append at the last index (shape invariant preserved), then sift-up (order invariant restored). The path from a leaf to the root has at most ⌊log₂ n⌋ steps, so the whole insert is O(log n)." },
      { type: "heapPlayground", kind: "min", seed: [3, 5, 8, 10, 12] },
      { type: "code", title: "Iterative sift-up", code:
`def push(a, x):
    a.append(x)
    i = len(a) - 1
    while i > 0:
      ...
      { type: "dryRun",
        ...
        caption: "Inserting 1 bubbles it from the last index up to the root — three swaps for a heap of six." },
      { type: "complexity", rows: [
        { op: "push", time: "O(log n)", space: "O(1)", note: "Iterative sift-up." },
      ]},
    ],
  },
  {
    ...
    readMinutes: 5,
    sections: [
      { type: "theory", text: "Removing the root cannot leave a hole, so we replace it with the last value in the array and pop the tail. The promoted value probably violates the order invariant — sift it down along the path of smaller children until both invariants hold." },
      { type: "heapPlayground", kind: "min", seed: [1, 3, 2, 7, 9, 5, 4] },
      { type: "code", title: "Iterative sift-down", code:
`def pop(a):
    top = a[0]
    last = a.pop()
    if a:
  ...
  },
  {
    ...
    readMinutes: 2,
    sections: [
      { type: "code", code: `def peek(a):\n    if not a: raise IndexError("peek from empty heap")\n    return a[0]` },
      { type: "callout", kind: "tip", title: "Common pattern",
  ...
  },
  {
    ...
    eyebrow: "Min Heap · 8",
    description: "The subroutine that restores order after an insertion — swap with parent while smaller.",
    difficulty: "Intermediate",
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Sift-up starts at any index and walks toward the root, swapping with the parent whenever the child is smaller. It terminates the moment the child is ≥ its parent, guaranteeing an O(log n) bound." },
      { type: "code", code:
`def sift_up(a, i):
    while i > 0:
        p = (i - 1) // 2
  ...
  },
  {
    ...
    eyebrow: "Min Heap · 9",
    description: "The subroutine that restores order after a deletion — swap with the smaller child while bigger.",
    difficulty: "Intermediate",
    readMinutes: 4,
    sections: [
      { type: "theory", text: "Sift-down starts at any index and walks toward a leaf. At each step it picks the smaller of the two children and swaps if the current node is bigger. Choosing the smaller child is essential — swapping with the bigger one would break the order invariant on the other subtree." },
      { type: "code", code:
`def sift_down(a, i):
    n = len(a)
    while True:
        ...
        a[i], a[s] = a[s], a[i]
        i = s` },
      { type: "callout", kind: "warn", title: "Pick the smaller child",
  ...
  },
  {
    ...
    eyebrow: "Min Heap · 10",
    description: "Turn an unsorted array into a heap in linear time by sift-down from the last internal node.",
    difficulty: "Intermediate",
    readMinutes: 5,
    sections: [
      { type: "theory", text: "The naïve approach — call push n times — costs O(n log n). The bottom-up construction is O(n): start at the last internal node (index n//2 − 1) and sift-down each node toward the root. Because most nodes are near the bottom (where sift-down does almost nothing), the total work is linear." },
      { type: "code", code:
`def build_heap(a):
  ...
  },
  {
    ...
    title: "Applications",
    eyebrow: "Min Heap · 12",
    description: "Where the min-heap is the right tool: Dijkstra, Huffman, k-way merge, event schedulers.",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      { type: "theory", bullets: [
  ...
  },
  {
    ...
    readMinutes: 3,
    sections: [
      { type: "table", headers: ["Advantage", "Details"], rows: [
      ...
      { type: "table", headers: ["Disadvantage", "Details"], rows: [
  ...
  },
  {
    ...
    title: "FAQ",
    eyebrow: "Min Heap · 14",
    description: "Fast answers to the questions everyone asks after implementing their first min-heap.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      { type: "faq", items: [
  ...
  },
  {
    ...
    eyebrow: "Min Heap · 15",
    description: "The questions interviewers reach for when they want to see if you can reason about heaps.",
    difficulty: "Intermediate",
    readMinutes: 3,
    sections: [
    ...
    sections: [
      { type: "practice", groups: [
        { level: "Beginner", items: [
          { title: "Implement Min Heap (from scratch)", url: "https://leetcode.com/problems/design-a-stack-with-increment-operation/", difficulty: "Easy", pattern: "Structure" },
          { title: "LC 703 · Kth Largest Element in a Stream", url: "https://leetcode.com/problems/kth-largest-element-in-a-stream/", difficulty: "Easy", pattern: "Min-heap of size k" },
        ]},
        { level: "Intermediate", items: [
          { title: "LC 215 · Kth Largest Element in an Array", url: "https://leetcode.com/problems/kth-largest-element-in-an-array/", difficulty: "Medium", pattern: "Top-K" },
          { title: "LC 973 · K Closest Points to Origin", url: "https://leetcode.com/problems/k-closest-points-to-origin/", difficulty: "Medium", pattern: "Top-K" },
        ]},
      ]},
    ],
  },
  {
    ...
    sections: [
      { type: "references", items: [
        ...
        { label: "GeeksforGeeks · Min Heap", url: "https://www.geeksforgeeks.org/min-heap-in-python/" },
