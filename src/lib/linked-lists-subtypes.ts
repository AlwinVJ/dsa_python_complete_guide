  ...
  return [
    {
      ...
      sections: [
        {
          type: "theory",
          text: `A ${name} is a chain of nodes where each node stores a value and ${
        ...
        },
        {
          ...
          variant: viz,
          nullTerminator: nl,
          cycleTo: isCirc ? 0 : undefined,
          caption: `A four-node ${name.toLowerCase()}.`,
        },
        {
          type: "theory",
          bullets: [
            hasPrev
              ? "Bidirectional traversal — forward AND backward in O(n)."
              : "Forward-only traversal.",
            isCirc
              ? "No `None` terminator — traversal must stop when `cur is head`."
              : "Terminates cleanly at `None`.",
            hasPrev
              ? "Deletion given a node reference is O(1)."
              : "Deletion requires the previous node — O(n) unless you already have it.",
            hasPrev
              ? `Higher memory cost — every node carries an extra pointer (${t === "circular-doubly" || t === "doubly" ? "24" : "16"} bytes on 64-bit CPython).`
        ...
        },
        {
          ...
          text:
            t === "singly"
              ? "Stack implementations, immutable functional lists (Lisp/Haskell), hash-table separate chaining."
              : t === "doubly"
                ? "LRU caches (dict + doubly linked list), collections.deque, browser back/forward history."
                : t === "circular"
                  ? "Round-robin schedulers, music playlists, Josephus problem, token-ring networks."
      ...
      sections: [
        {
          ...
          bullets: [
            ...
            hasPrev && isCirc
              ? "The head's `prev` is the tail — both ends are reachable in O(1)."
              : "",
          ].filter(Boolean),
        },
        {
    ...
    },
    {
      ...
      sections: [
        {
          type: "theory",
          text: `Each node is an independent heap object. ${hasPrev ? "Two 8-byte pointers per node instead of one" : "One 8-byte pointer per node"}. Cache locality is poor either way — every pointer chase can miss.`,
        },
...
head = 0x100${hasPrev && isCirc ? "  # tail = head.prev = 0x188" : ""}`,
    ...
    },
    {
      ...
      sections: [
        {
          type: "theory",
          text: `Every operation on a ${name.toLowerCase()} boils down to reading and writing \`.${NEXT[t]}\`${hasPrev ? " / `.prev`" : ""} fields. Drawing the pointer diagram before coding is the single strongest habit you can build.`,
        },
        {
          type: "viz",
          nodes: [1, 2, 3],
    ...
    },
    {
      ...
      sections: [
        ...
        },
        {
          ...
          cycleTo: isCirc ? 0 : undefined,
          highlight: [0, 4],
          caption: "After push_front(5) + push_back(99)",
        },
        { type: "playground", initial: [10, 20, 30] },
        {
          ...
          rows: [
            {
              op: "push_front",
              time: t === "circular" ? "O(n)" : "O(1)",
              note: t === "circular" ? "must locate tail" : "",
            },
            {
              op: "push_back",
              time:
    ...
    },
    {
      ...
      sections: [
        {
          type: "dryRun",
          headers: ["step", "operation", "list state"],
          rows: [
            ["1", "push_front(5)", "5 → …"],
      ...
      sections: [
        {
          ...
          rows: [
            ...
            {
              op: "Delete at tail",
              time: hasPrev && isCirc ? "O(1)" : hasPrev ? "O(1) w/ tail" : "O(n)",
            },
            { op: "Reverse", time: "O(n)" },
      ...
      sections: [
        {
          ...
          items: [
            ...
            ...(hasPrev
              ? [
                  "Updating `next` but forgetting the matching `prev` link — silently breaks reverse traversal.",
                ]
              : []),
            ...(isCirc
              ? [
    ...
    },
    {
      ...
      sections: [
        {
          type: "tip",
          text: "Draw the list BEFORE you write the loop — most bugs are pointer-order bugs.",
        },
        {
          type: "callout",
          kind: "interview",
          title: "Standard follow-ups",
          text:
            t === "singly"
              ? "Reverse in groups of k · Detect a cycle · Merge two sorted lists · Palindrome check in O(1) space."
              : t === "doubly"
                ? "Design an LRU cache · Flatten a multilevel doubly linked list · Insertion Sort a DLL."
                : t === "circular"
                  ? "Josephus problem · Split a circular list into two halves · Detect the loop's starting node."
    ...
    },
    {
      ...
      sections: [
        {
          type: "theory",
          text: `Q: When should I choose a ${name.toLowerCase()} over a Python list?`,
        },
        {
          type: "theory",
          text:
            t === "singly"
              ? "A: When you need cheap front-inserts and don't care about random indexing — e.g. persistent immutable structures, undo stacks, hash-table chaining."
              : t === "doubly"
                ? "A: When you need O(1) deletion given a node reference AND O(1) pop-from-either-end — the LRU-cache pattern."
                : t === "circular"
                  ? "A: When your workload naturally wraps around — round-robin schedulers, playlists, ring buffers."
        ...
        {
          type: "theory",
          text: "A: No — CPython `list` is a resizable dynamic array. For deque-like linked storage use `collections.deque` (implemented as a doubly linked list of blocks).",
        },
        { type: "theory", text: `Q: What's the memory overhead vs. an array?` },
        {
          type: "theory",
          text: `A: A ${hasPrev ? "doubly" : "singly"} linked node adds ${hasPrev ? "≈ 24" : "≈ 16"} bytes of pointers per element on top of the value. A Python list stores a single 8-byte reference per element.`,
        },
      ],
    },
    {
              ...
              items:
                t === "doubly"
                  ? [
                      ...
                      {
                        title: "430 · Flatten Multilevel DLL",
                  ...
                  : t === "circular"
                    ...
                    : t === "circular-doubly"
                      ? [
                          {
                            title: "1290 · Convert Binary Number in LL to Integer",
          ...
          items: [
            ...
            },
            {
              ...
              choices: [
                "next only",
                "prev only",
                hasPrev ? "prev AND next" : "value only",
                "index",
              ],
              answer: hasPrev ? 2 : 0,
            },
            ...
            },
            {
              ...
              explain:
                t === "circular"
                  ? "Plain circular singly needs an O(n) walk to find the tail."
                  : "Front-insert is a couple of pointer writes.",
            },
            {
...
export const LL_SUBTYPES: {
...
}[] = [
  {
    ...
    tagline: "One-way chain of nodes.",
    lessons: lessons("singly"),
  },
  {
    slug: "doubly",
    title: "Doubly Linked List",
    tagline: "Bidirectional pointers for O(1) either-end ops.",
    lessons: lessons("doubly"),
  },
  {
    slug: "circular",
    title: "Circular Linked List",
    tagline: "Tail loops back to head — perfect for round-robin.",
    lessons: lessons("circular"),
  },
  {
    slug: "circular-doubly",
    title: "Circular Doubly Linked List",
    tagline: "Rings with both-way traversal.",
    lessons: lessons("circular-doubly"),
  },
];
