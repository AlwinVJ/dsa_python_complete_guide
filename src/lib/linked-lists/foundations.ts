...
export const LL_FOUNDATIONS: LLLesson[] = [
  {
    ...
    sections: [
      {
        type: "theory",
        text: "A linked list is a linear data structure where each element (a node) stores a value plus a pointer to the next element. Unlike arrays, the elements live at arbitrary memory locations — they only appear ordered because each node knows where the next one lives.",
      },
      {
        type: "viz",
        nodes: [10, 20, 30, 40],
        headLabel: "HEAD",
        caption: "A 4-node singly linked list.",
      },
      {
        type: "theory",
        bullets: [
      ...
      },
      {
        type: "callout",
        kind: "info",
        title: "You will meet 4 variants",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Arrays are wonderfully cache-friendly but rigid: a fixed contiguous block of memory. When you need to insert or delete in the middle, arrays force you to shift every element after it. Linked lists trade random access for cheap mid-sequence structural changes.",
      },
      {
        type: "theory",
        bullets: [
    ...
    sections: [
      {
        ...
        rows: [
          {
            op: "Access by index",
            time: "Array O(1) · LinkedList O(n)",
            note: "Arrays win big for random access.",
          },
          ...
          {
            op: "Memory layout",
            time: "Contiguous · Scattered",
            note: "Cache locality favors arrays.",
          },
          {
            op: "Extra memory per element",
            time: "0 · pointer(s)",
            note: "LinkedList pays for links.",
          },
        ],
      },
    ],
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "A node is a small container object that holds a value and one or more references to other nodes. The reference (also called a link or pointer) is what stitches independent nodes into a sequence.",
      },
      {
        type: "code",
        title: "smallest possible node",
        code: `class Node:
    __slots__ = ("val", "next")
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        type: "callout",
        kind: "did",
        title: "The payload is arbitrary",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Nodes live wherever the allocator finds free space. The values look sequential only because each node's `next` field stores the address of the next allocated block.",
      },
      {
        type: "code",
        title: "conceptual memory dump",
...
next:  0x240    next:  0x580    next:  NULL`,
      },
      {
        type: "memory",
        nodes: [10, 20, 30],
        caption: "Nodes are scattered; each stores the address of the next.",
      },
      {
        type: "callout",
        kind: "perf",
  ...
  },
  {
    ...
    description:
      "In Python, every variable is a reference. Understand that or debugging linked lists is a nightmare.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "Python doesn't expose raw pointers, but every variable holds a reference to an object. Assigning `b = a` does not copy the node — both names now point to the same object.",
      },
      {
        type: "code",
...
print(a.val)        # 99 — a was mutated too`,
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "The `head` pointer is the entry point. Lose it and you lose the whole list — Python's garbage collector will free every unreachable node. Almost every algorithm walks starts from `head`.",
      },
      {
        type: "viz",
        nodes: [10, 20, 30],
        headLabel: "HEAD",
        caption: "Head refers to the first node.",
      },
      {
        type: "callout",
        kind: "tip",
        title: "Always update head",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Without a tail pointer, appending forces a full traversal. Storing a second reference — `tail` — lets you append in constant time. The tradeoff: every insert / delete must maintain that pointer correctly.",
      },
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "The last node's `next` field is `None` in Python (equivalent to NULL in C). Every traversal loop tests `while cur is not None` — if you forget the terminator you get an AttributeError, and if you forget the check you loop forever.",
      },
      {
        type: "code",
        title: "the terminator check",
        code: `cur = head
while cur is not None:      # None marks the end
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Traversal always follows the same shape: start at `head`, do something with the current node, advance via `cur = cur.next`. This pattern is the atomic building block for search, count, reverse, and almost every other algorithm.",
      },
      { type: "viz", nodes: [10, 20, 30, 40], highlight: [0], caption: "cur starts at head" },
      {
        ...
        nodes: [10, 20, 30, 40],
        highlight: [2],
        caption: "cur has advanced two hops",
      },
    ],
  },
  {
...
head = a         # entry point`,
      },
      {
        ...
        nodes: [10, 20, 30],
        headLabel: "HEAD",
        caption: "The result of wiring three nodes together.",
      },
    ],
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Every `Node(...)` call reserves memory on Python's heap. There is no pre-allocated buffer. This is the opposite of a Python list, which grows in doubling chunks and occasionally copies everything.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        type: "theory",
        text: "The table above lays out how the four variants differ across every axis that matters. Use it as a lookup when you're picking a structure for a new problem.",
      },
    ],
  },
  {
    ...
    sections: [
      ...
      {
        type: "theory",
        text: "Use the playground above to build intuition. Try: create three nodes, connect A→B and B→C, then move the HEAD chip onto A. Notice how the simulated addresses stay stable while only the `next` fields change.",
      },
      {
        type: "callout",
        kind: "tip",
