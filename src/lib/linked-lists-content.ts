...
export type LLPractice = {
  ...
  groups: {
    ...
    items: { title: string; url: string; difficulty: "Easy" | "Medium" | "Hard"; pattern?: string; time?: string }[];
  }[];
};
...
export type LLSection =
  | LLViz | LLMemory | LLTheory | LLCode | LLDryRun | LLPlayground | LLComplexity
  | LLMistakes | LLTip | LLCallout | LLQuiz | LLPractice | LLRefs | LLInterview | LLHeading;
...
export const LL_LESSONS: LLLesson[] = [
  // 1
  {
    ...
    sections: [
      { type: "theory", text:
        "Unlike a Python list — which is a contiguous block of pointers — a linked list scatters its nodes across the heap. Each node holds two things: its value, and a reference (`next`) to the next node. The list is identified by a single `head` pointer; when `head is None`, the list is empty." },
      { type: "viz", nodes: [10, 20, 30, 40], headLabel: "HEAD", caption: "A four-node singly linked list." },
      { type: "theory", bullets: [
      ...
      { type: "callout", kind: "did", title: "Did you know?",
      ...
      { type: "quiz", items: [
        { q: "What does a linked-list `head` point to?", choices: ["The last node", "The first node", "The middle node", "The largest value"], answer: 1,
          explain: "`head` references the first node; the list is walked by following `next` pointers." },
      ]},
    ],
  },
  ...
  // 2
  {
    ...
    sections: [
      { type: "theory", text:
        "Arrays are amazing at random access but pay for it. Inserting at the front of a Python list is O(n) because every element shifts. If your workload is dominated by head-inserts, tail-appends, and 'delete this node' operations, a linked list wins." },
    ...
    sections: [
      ...
      { type: "quiz", items: [
        { q: "You need to insert 1000 items at the front of a collection. Which is faster?",
          choices: ["Python list", "Singly linked list", "Both same"], answer: 1,
          explain: "Each `list.insert(0, x)` is O(n); linked-list push-front is O(1) — 1000× vs 1000² operations." },
      ]},
    ],
  },
  ...
  // 4
  {
    ...
    sections: [
      { type: "theory", text:
        "Every `Node` is an independent heap object. The `next` field is just a reference (essentially an 8-byte pointer on 64-bit CPython). Traversing the list dereferences one pointer at a time; each jump can be a cache miss." },
...
head = 0x100` },
  ...
  // 5
  {
    ...
    readMinutes: 4,
    sections: [
      { type: "code", title: "singly linked node", code:
`class Node:
    __slots__ = ("val", "next")
    ...
    def __repr__(self):
      ...
      { type: "code", title: "doubly linked node", code:
`class DNode:
    ...
    def __init__(self, val, prev=None, nxt=None):
      ...
      { type: "quiz", items: [
        { q: "What is the default value of `next` in a freshly created `Node(5)`?",
          choices: ["0", "None", "Node(0)", "undefined"], answer: 1 },
      ]},
    ],
  },
...
`def from_iterable(values):
    ...
    return dummy.next` },
      ...
      { type: "code", code:
`def from_iterable_reversed(values):
    head = None
...
`def traverse(head):
    ...
    while cur is not None:
        print(cur.val)
        cur = cur.next` },
      { type: "dryRun", headers: ["step", "cur", "cur.val"], rows: [
        ["1", "Node(10)", "10"],
        ["2", "Node(20)", "20"],
        ["3", "Node(30)", "30"],
        ["4", "Node(40)", "40"],
...
`def find(head, target):
    ...
    while head:
        ...
        idx += 1
    return -1` },
      { type: "viz", nodes: [5, 8, 12, 21, 34], highlight: [2], caption: "find(head, 12) → returns 2" },
      { type: "complexity", rows: [
      ...
      { type: "tip", text: "Unlike sorted arrays, a linked list can't use binary search — random access is O(n), which cancels out log-n gains." },
    ],
  },
...
`def push_front(head, val):
    return Node(val, head)   # new node's next = old head` },
      { type: "dryRun", headers: ["step", "action"], rows: [
  ...
  // 10
  {
    ...
    readMinutes: 6,
    sections: [
      { type: "code", title: "no tail pointer — O(n)", code:
`def push_back(head, val):
    ...
    cur.next = node
    return head` },
      { type: "code", title: "maintained tail pointer — O(1)", code:
`class LinkedList:
    def __init__(self):
...
`def insert_at(head, k, val):
    ...
    return dummy.next` },
      ...
      { type: "viz", nodes: [10, 20, 30, 40, 50], highlight: [2], caption: "insert_at(head, 2, 30)" },
      { type: "dryRun", headers: ["k", "prev", "prev.next", "action"], rows: [
        ["0", "dummy", "10", "walk"],
        ["1", "Node(10)", "20", "walk"],
...
`def pop_back(head):
    ...
    cur.next = None
    return head` },
      { type: "viz", nodes: [10, 20, 30, 40], compare: [2], caption: "Stop at the 2nd-to-last node." },
...
`def delete_value(head, target):
    ...
    return dummy.next` },
      ...
      { type: "viz", nodes: [10, 20, 30, 40], compare: [1], caption: "delete_value(head, 20) — locate" },
      { type: "viz", nodes: [10, 30, 40], caption: "After unlink" },
      { type: "code", title: "delete every occurrence", code:
...
`def update(head, idx, val):
  ...
  // 16
  {
    ...
    description: "The single most-asked linked-list interview question. In-place, one pass, three pointers.",
    difficulty: "Intermediate",
    readMinutes: 7,
    sections: [
      { type: "theory", text:
        "Walk the list once, and at every step flip the current node's `next` pointer to point at the previously visited node. Three pointers — `prev`, `cur`, `nxt` — are enough. Return `prev` as the new head." },
      { type: "viz", nodes: [1, 2, 3, 4], caption: "Before" },
      { type: "viz", nodes: [4, 3, 2, 1], highlight: [0], caption: "After reverse" },
      { type: "code", title: "iterative", code:
`def reverse(head):
    prev, cur = None, head
    while cur:
        ...
        cur = nxt          # advance cur
    return prev` },
      { type: "dryRun", headers: ["iter", "prev", "cur", "cur.next after flip"], rows: [
        ["0", "None", "1", "None"],
        ["1", "1", "2", "1"],
        ["2", "2", "3", "2"],
        ["3", "3", "4", "3"],
      ...
      { type: "code", title: "recursive", code:
`def reverse_rec(head):
    ...
    return new_head` },
      ...
      { type: "callout", kind: "interview", title: "Follow-up questions",
...
`def middle(head):
    ...
    while fast and fast.next:
        ...
        fast = fast.next.next
    return slow` },
      { type: "viz", nodes: [1, 2, 3, 4, 5], slowIdx: 2, fastIdx: 4, caption: "After the loop for odd length: slow is exactly the middle." },
      { type: "viz", nodes: [1, 2, 3, 4], slowIdx: 2, fastIdx: 3, caption: "Even length: slow is the SECOND middle. Use `while fast.next and fast.next.next` for the first middle." },
  ...
  // 18
  {
    ...
    sections: [
      { type: "theory", text:
        "If there's no cycle, `fast` will hit `None`. If there IS a cycle, `fast` laps `slow` and they meet. This is the celebrated tortoise-and-hare algorithm." },
      { type: "viz", nodes: [3, 2, 0, -4], cycleTo: 1, slowIdx: 2, fastIdx: 2, caption: "They meet at node index 2." },
      { type: "code", code:
`def has_cycle(head):
    slow = fast = head
    while fast and fast.next:
  ...
  // 19
  {
    ...
    sections: [
      { type: "theory", text:
        "After slow/fast meet inside the cycle, reset one pointer to `head` and advance both by 1. The point where they meet again is the cycle's entry. Then walk to the node whose `next` is that entry and set `.next = None`." },
      { type: "code", code:
`def detect_start(head):
    slow = fast = head
    while fast and fast.next:
        slow = slow.next
        fast = fast.next.next
...
def remove_cycle(head):
    ...
    cur.next = None
    return head` },
      { type: "viz", nodes: [3, 2, 0, -4], cycleTo: 1, highlight: [1], caption: "Cycle entry = node at index 1." },
      { type: "complexity", rows: [{ op: "detect + remove", time: "O(n)", space: "O(1)" }] },
      { type: "tip", text: "Prove it on paper: let L = distance head→entry, C = cycle length, and show slow travels L + kC to meet fast." },
    ],
  },
...
`def merge(a, b):
    ...
    return dummy.next` },
      ...
      { type: "dryRun", headers: ["a", "b", "chosen", "tail"], rows: [
        ["1→4→8", "2→3→9", "1", "1"],
        ["4→8", "2→3→9", "2", "1→2"],
        ["4→8", "3→9", "3", "1→2→3"],
        ["4→8", "9", "4", "1→2→3→4"],
  ...
  // 21
  {
    ...
    sections: [
      { type: "theory", text:
        "Quick sort needs random access to partition efficiently, so it degrades on linked lists. Merge sort — split by finding the middle, recurse, merge — hits O(n log n) time and O(log n) recursion space, no swaps." },
      { type: "code", code:
`def sort_list(head):
    if head is None or head.next is None:
        return head
  ...
  // 24
  {
    ...
    sections: [
      { type: "theory", text:
        "A Python `Node` with `__slots__ = ('val', 'next')` weighs ~56 bytes on 64-bit CPython (16 bytes object header + 2 × 8-byte pointers + object padding). A dynamic array pays only 8 bytes per element (a single reference)." },
      { type: "complexity", rows: [
  ...
  // 26
  {
    ...
    sections: [
      ...
      { type: "callout", kind: "interview", title: "Signals interviewers look for",
  ...
  // 27
  {
    ...
    sections: [
      ...
      { type: "theory", text: "A: No. `list` is a resizable dynamic array — O(1) indexing, O(n) mid-insert. For a doubly-linked queue-like structure use `collections.deque`." },
      { type: "theory", text: "Q: Should I ever build my own linked list in production Python?" },
      { type: "theory", text: "A: Rarely. `deque` covers 95% of use cases. Write your own only for interview practice or when implementing custom cache eviction, intrusive lists, or lock-free structures in C-extensions." },
      { type: "theory", text: "Q: Why is push_back O(n) here but O(1) on Python lists?" },
      { type: "theory", text: "A: Python lists over-allocate their backing array and pay amortised O(1) for append. A linked list without a tail pointer has to walk the whole chain." },
      { type: "theory", text: "Q: What's the practical difference between singly and doubly linked?" },
      { type: "theory", text: "A: Doubly gives you O(1) deletion given the node and O(1) pop_back; singly saves one pointer per node. Choose doubly when either matters." },
      { type: "theory", text: "Q: Can I use recursion to solve every linked-list problem?" },
      { type: "theory", text: "A: Yes structurally, but you pay O(n) stack space and risk `RecursionError` on long lists (CPython's default limit is 1000). Prefer iterative solutions in interviews unless recursion is clearly cleaner." },
    ],
  },
    ...
    sections: [
      { type: "practice", groups: [
        { level: "Beginner", items: [
          ...
          { title: "141 · Linked List Cycle", url: "https://leetcode.com/problems/linked-list-cycle/", difficulty: "Easy", pattern: "Floyd's", time: "15m" },
          ...
          { title: "21 · Merge Two Sorted Lists", url: "https://leetcode.com/problems/merge-two-sorted-lists/", difficulty: "Easy", pattern: "Dummy head", time: "15m" },
          { title: "83 · Remove Duplicates from Sorted List", url: "https://leetcode.com/problems/remove-duplicates-from-sorted-list/", difficulty: "Easy", pattern: "One pass", time: "10m" },
        ]},
        { level: "Intermediate", items: [
          { title: "19 · Remove Nth From End", url: "https://leetcode.com/problems/remove-nth-node-from-end-of-list/", difficulty: "Medium", pattern: "Two pointers", time: "20m" },
          { title: "142 · Linked List Cycle II", url: "https://leetcode.com/problems/linked-list-cycle-ii/", difficulty: "Medium", pattern: "Floyd's + entry", time: "25m" },
          { title: "234 · Palindrome Linked List", url: "https://leetcode.com/problems/palindrome-linked-list/", difficulty: "Easy", pattern: "Reverse half", time: "25m" },
          { title: "2 · Add Two Numbers", url: "https://leetcode.com/problems/add-two-numbers/", difficulty: "Medium", pattern: "Carry & merge", time: "25m" },
          { title: "148 · Sort List", url: "https://leetcode.com/problems/sort-list/", difficulty: "Medium", pattern: "Merge sort", time: "30m" },
          { title: "138 · Copy List with Random Pointer", url: "https://leetcode.com/problems/copy-list-with-random-pointer/", difficulty: "Medium", pattern: "Interleave / hash", time: "30m" },
        ]},
        { level: "Advanced", items: [
          { title: "23 · Merge k Sorted Lists", url: "https://leetcode.com/problems/merge-k-sorted-lists/", difficulty: "Hard", pattern: "Min-heap", time: "35m" },
          { title: "25 · Reverse Nodes in k-Group", url: "https://leetcode.com/problems/reverse-nodes-in-k-group/", difficulty: "Hard", pattern: "Group reverse", time: "40m" },
          { title: "146 · LRU Cache", url: "https://leetcode.com/problems/lru-cache/", difficulty: "Medium", pattern: "DLL + hashmap", time: "45m" },
          { title: "460 · LFU Cache", url: "https://leetcode.com/problems/lfu-cache/", difficulty: "Hard", pattern: "DLL of DLLs", time: "60m" },
        ]},
      ]},
    ],
  },
    ...
    sections: [
      { type: "quiz", items: [
        ...
        { q: "After `head = Node(1, Node(2, Node(3)))`, what is `head.next.next.val`?",
          choices: ["1", "2", "3", "None"], answer: 2 },
        { q: "Which pattern gives the middle node in one pass?",
          choices: ["Binary search", "Slow/fast pointers", "Recursion + counter", "Hashing"], answer: 1 },
        { q: "In `reverse`, why do we save `nxt = cur.next` first?",
          choices: ["It's faster", "Otherwise we lose access to the rest of the list", "To satisfy Python's assignment rules", "For readability only"], answer: 1 },
        { q: "You call `pop_back` on a singly linked list of length 10⁶. Roughly how many pointer dereferences?",
          choices: ["1", "log 10⁶", "10⁶", "10⁶ · log 10⁶"], answer: 2,
          explain: "You must walk to the second-to-last node." },
        { q: "Which of these is NOT solvable in O(1) extra space?",
          choices: ["Reverse iteratively", "Detect cycle", "Merge two sorted lists", "Sort with merge sort (naive recursion)"], answer: 3,
        ...
        { q: "You forgot to add `dummy = Node(0, head)` before deleting by value. Which case now breaks?",
          choices: ["Empty list", "Deleting the head", "Deleting the tail", "Duplicate values"], answer: 1 },
        { q: "What does Floyd's algorithm return when the list has no cycle?",
          choices: ["The head", "None / False", "The tail", "Infinite loop"], answer: 1 },
        { q: "In a merge of `[1,3]` and `[2]`, how many comparisons does the loop perform?",
          choices: ["1", "2", "3", "4"], answer: 1,
          explain: "Compare 1 vs 2 (pick 1), compare 3 vs 2 (pick 2). Then b is empty and remainder is attached." },
        { q: "Choosing between a linked list and a Python list for a stack, which is better and why?",
          choices: ["Linked — O(1) push/pop", "Python list — O(1) amortised push/pop with better cache locality", "Same performance", "Depends on the OS"], answer: 1 },
      ]},
    ],
  },
  ...
  // 30
  {
    ...
    sections: [
      { type: "references", items: [
        ...
        { label: "CPython source · _collectionsmodule.c", url: "https://github.com/python/cpython/blob/main/Modules/_collectionsmodule.c" },
        ...
        { label: "MIT 6.006 · Linked lists (lecture notes)", url: "https://ocw.mit.edu/courses/6-006-introduction-to-algorithms-spring-2020/" },
        { label: "Linux kernel intrusive list", url: "https://www.kernel.org/doc/html/latest/core-api/kernel-api.html#list-management-functions" },
        ...
        { label: "HackerRank · Linked List domain", url: "https://www.hackerrank.com/domains/data-structures?filters%5Bsubdomains%5D%5B%5D=linked-lists" },
      ]},
    ],
  },
];
