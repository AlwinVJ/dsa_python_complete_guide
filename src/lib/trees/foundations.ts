...
export const T_FOUNDATIONS: TLesson[] = [
  {
    ...
    description:
      "A tree is a hierarchical, acyclic collection of nodes — the shape behind file systems, DOMs, parsers, and database indexes.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "A tree is a connected, acyclic graph with a single designated root. Every node has zero or more children and exactly one parent (except the root). Trees model any 'is contained by / belongs to' relationship, from folders on a disk to the parse tree a compiler builds from your source code.",
      },
      {
        type: "tree",
        root: {
  ...
  },
  {
    ...
    description:
      "The problems that arrays and linked lists cannot solve cheaply — and how a tree shape fixes them.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
  ...
  },
  {
    ...
    sections: [
      {
        type: "table",
        headers: ["Structure", "Access", "Search", "Insert", "Weakness"],
        rows: [
      ...
      {
        type: "theory",
        text: "Trees trade a little constant-factor overhead for logarithmic behaviour on the hard operations. That trade is why every database index in production is a tree.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    description:
      "The vocabulary every tree lesson assumes: root, leaf, edge, depth, height, degree.",
    difficulty: "Beginner",
    readMinutes: 6,
    sections: [
      {
      ...
      },
      {
        ...
        rows: [
          ...
          [
            "Ancestor / Descendant",
            "Anything on the path up to the root / anything in the subtree.",
          ],
  ...
  },
  {
    ...
    description:
      "Invariants that every tree respects — n nodes have n-1 edges, and other useful facts.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
  ...
  },
  {
    ...
    description:
      "How Python actually lays a tree out in memory — pointers, arrays, and the trade-offs.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "code",
        title: "node objects (the default)",
        code: `class TreeNode:
...
#         └─ each field is a Python object reference`,
      ...
      {
        type: "code",
        title: "array-backed (heap layout)",
        code: `# For a *complete* binary tree, index 0 is root, and
...
#         0  1  2  3  4  5  6`,
      ...
      {
        type: "table",
        headers: ["Layout", "Pros", "Cons"],
        rows: [
          [
            "Node objects (pointers)",
            "Any shape, cheap insertion, easy code",
            "Extra bytes per node, pointer chase",
          ],
          [
            "Array (heap)",
            "Cache-friendly, no pointers, index math",
            "Wastes memory on sparse trees",
          ],
  ...
  },
  {
    ...
    description:
      "Depth-first (preorder / inorder / postorder) vs breadth-first (level order) — the four ways to visit every node.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "A traversal is a total ordering of every node in the tree. Depth-first uses recursion or an explicit stack; breadth-first uses a queue. Every algorithm you meet in the Algorithms tier is built from one of these four.",
      },
      {
        type: "table",
        ...
        rows: [
          ...
          [
            "Postorder",
            "Left → Right → Root",
            "Delete children before parent, evaluate expressions",
          ],
  ...
  },
  {
    ...
    description:
      "Every tree operation reduces to 'do the thing to the left subtree, do the thing to the right subtree, combine'.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "Trees are defined recursively — a tree is either empty, or a root plus a list of subtrees, each itself a tree. That recursive definition makes recursion the natural implementation.",
      },
      {
        type: "code",
        title: "the template",
  ...
  },
  {
    ...
    description:
      "Constraining every node to at most two children unlocks compact representations and simpler algorithms.",
    difficulty: "Beginner",
    readMinutes: 3,
    sections: [
      {
  ...
  },
  {
    ...
    description:
      "Insert, delete, search, and inspect metrics on a live BST — internalise the vocabulary before moving to variants.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "The playground is a real Binary Search Tree. Every insert grows the shape; every search highlights the path from the root. Watch the metrics (nodes, leaves, height, levels) update as you mutate it, and switch traversal modes to see the four orders side by side.",
      },
      { type: "playground" },
      {
        type: "callout",
    ...
    sections: [
      {
        ...
        rows: [
          {
            op: "Traversal (any order)",
            time: "O(n)",
            space: "O(h)",
            note: "h = height, recursion stack",
          },
