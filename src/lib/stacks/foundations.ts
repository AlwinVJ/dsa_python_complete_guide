...
export const STACK_FOUNDATIONS: StackLesson[] = [
  {
    ...
    description:
      "A stack is a LIFO container — the last item you push is the first one you pop. Build the mental model before touching code.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      {
        type: "theory",
        text: "A stack is a linear data structure where every insertion and removal happens at one end — the top. That single-end restriction is what makes the API tiny and impossible to misuse.",
      },
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "Formally, a stack is an Abstract Data Type (ADT) that supports two mutating operations — push(x) and pop() — plus a read-only peek()/top(). The order of removal is exactly the reverse of the order of insertion.",
      },
      {
        type: "code",
        title: "the ADT contract",
        code: `push(x)   -> None      # add x on top
...
size()    -> int`,
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "LIFO means the most recently pushed item is the next one popped. Picture a stack of plates: you can only add to or take from the top.",
      },
      ...
      { type: "viz", items: ["A", "B", "C"], caption: "push('C')" },
      {
        type: "viz",
        items: ["A", "B"],
        caption: "pop() → 'C' — the last one in is the first one out.",
      },
      {
        type: "callout",
        kind: "did",
        title: "Contrast with FIFO",
  ...
  },
  {
    ...
    sections: [
      {
        type: "theory",
        text: "An array-backed stack occupies one contiguous memory block; the CPU cache loves it. A linked-list-backed stack scatters nodes across the heap and follows one pointer per pop.",
      },
      {
        type: "viz",
        items: [10, 20, 30],
  ...
  },
  {
    ...
    description:
      "Every stack — hardware or software — is really 'a chunk of memory plus one integer'.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      {
        type: "theory",
        text: "The stack pointer (SP) is an index or address that says 'the next free slot is here'. Push writes then increments SP; pop decrements SP then reads. That's the whole trick.",
      },
      {
        type: "code",
        title: "SP-based push/pop",
        code: `# SP always points to the next free slot
buf = [None] * 8
sp = 0
...
def pop():
    ...
    return buf[sp]`,
      },
      {
        type: "callout",
        kind: "did",
        title: "Real CPUs work like this",
  ...
  },
  {
    ...
    description:
      "Push, pop, and peek in real time. Watch the top pointer move and simulated addresses update.",
    difficulty: "Beginner",
    readMinutes: 6,
    sections: [
    ...
    sections: [
      {
        ...
        rows: [
          {
            op: "Access at index k",
            time: "Array O(1) · Stack O(k)",
            note: "Stack must pop to reach k.",
          },
          { op: "Insert at end", time: "Array O(1)* · Stack O(1)" },
          {
            op: "Insert anywhere else",
            time: "Array O(n) · Stack N/A",
            note: "Stack has no 'middle'.",
          },
