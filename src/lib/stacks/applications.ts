...
export const STACK_APPLICATIONS: StackLesson[] = [
  // ---------------- Applications ----------------
  {
    ...
    description: "Two stacks — one for actions, one for undone actions — power every editor's undo history.",
    difficulty: "Beginner",
    readMinutes: 5,
    sections: [
      { type: "theory", text:
        "Every action the user takes is pushed onto an `undo` stack. Ctrl+Z pops from undo and pushes onto `redo`. Ctrl+Y does the reverse. A new action clears the redo stack — you can't 'redo' after diverging." },
      { type: "code", code:
`class History:
    def __init__(self):
        self.undo, self.redo = [], []
  ...
  },
  {
    ...
    sections: [
      { type: "theory", text:
        "Navigating to a new URL pushes onto the `back` stack and clears `forward`. Back pops from `back` and pushes onto `forward`. Forward does the mirror." },
    ],
  },
  {
    ...
    sections: [
      { type: "theory", text:
        "A stack frame holds the function's local variables and the return address. When main() calls foo(), the CPU pushes foo's frame. When foo returns, the frame is popped and execution jumps to the return address." },
      { type: "callout", kind: "warn", title: "RecursionError",
  ...
  },
  {
    ...
    description: "Convert infix to postfix (Shunting-yard) and evaluate postfix — both are stack-driven.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      { type: "code", title: "evaluate postfix (RPN)", code:
...
`def valid(s):
    ...
    for c in s:
        if c in "([{":
            ...
            return False
    return not st` },
      { type: "dryRun", headers: ["char", "stack", "action"], rows: [
        ["(", "[(]",  "push"],
        ["[", "[(, []", "push"],
        ["]", "[(]",  "pop matches"],
        [")", "[]",   "pop matches"],
  ...
  },
  {
    ...
    description: "Sudoku, N-queens, maze solvers — every backtracking algorithm is a stack of choices.",
    difficulty: "Intermediate",
    readMinutes: 4,
    sections: [
      { type: "theory", text:
        "Each recursive call pushes a partial choice onto the call stack; when it dead-ends, we return (pop) and try the next option. An explicit stack turns this into an iterative loop that never blows the recursion limit." },
    ],
  },
  {
    slug: "dfs",
    ...
    description: "DFS explores as deep as possible before backtracking — a stack encodes exactly that discipline.",
    difficulty: "Intermediate",
    readMinutes: 5,
    sections: [
      { type: "code", title: "iterative DFS", code:
`def dfs(graph, start):
    ...
    while stack:
        ...
        for nxt in reversed(graph[v]):
            if nxt not in visited:
                stack.append(nxt)
    return visited` },
      { type: "callout", kind: "did", title: "BFS vs DFS in code",
...
reverse("stack")   # 'kcats'` },
  ...
  },
  {
    ...
    description: "Reverse a stack in place using only recursion and push/pop — no auxiliary container.",
    difficulty: "Intermediate",
    readMinutes: 5,
    sections: [
      { type: "code", code:
...
def reverse_stack(st):
    ...
    insert_bottom(st, top)` },
      { type: "complexity", rows: [{ op: "reverse_stack", time: "O(n²)", space: "O(n) recursion" }] },
      { type: "callout", kind: "interview", title: "Classic follow-up",
...
`def sort_stack(st):
  ...
  },
  {
    ...
    description: "Push the first half, then compare with the second half — a linear-time palindrome test.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      { type: "code", code:
...
`def is_valid(s):
    ...
    return not st` },
      { type: "practice", groups: [
        { level: "Beginner", items: [
          { title: "LC 20 · Valid Parentheses", url: "https://leetcode.com/problems/valid-parentheses/", difficulty: "Easy", pattern: "Matching" },
        ]},
      ]},
    ],
  },
  {
    ...
    sections: [
      { type: "theory", text:
        "Store pairs (value, current_min) at each level, or use two parallel stacks: one for values and one for the running minimum." },
      { type: "code", code:
`class MinStack:
    ...
    def getMin(self):  return self.s[-1][1]` },
      { type: "callout", kind: "interview", title: "Also works for getMax",
        text: "Same trick — store (value, running_max). The pattern is called 'auxiliary stack'." },
      { type: "practice", groups: [
        { level: "Intermediate", items: [
          { title: "LC 155 · Min Stack", url: "https://leetcode.com/problems/min-stack/", difficulty: "Medium", pattern: "Auxiliary Stack" },
        ]},
      ]},
    ],
  },
...
`class Queue:
    ...
    def dequeue(self):
      ...
      { type: "practice", groups: [{ level: "Beginner", items: [
        { title: "LC 232 · Implement Queue using Stacks", url: "https://leetcode.com/problems/implement-queue-using-stacks/", difficulty: "Easy" },
      ]}]},
    ],
  },
  {
    ...
    description: "Remove pairs of adjacent duplicate characters — a stack collapses them naturally.",
    difficulty: "Beginner",
    readMinutes: 4,
    sections: [
      { type: "code", code:
...
remove_dupes("abbaca")   # 'ca'` },
      { type: "practice", groups: [{ level: "Beginner", items: [
        { title: "LC 1047 · Remove All Adjacent Duplicates", url: "https://leetcode.com/problems/remove-all-adjacent-duplicates-in-string/", difficulty: "Easy" },
      ]}]},
    ],
  },
  {
    ...
    description: "For each item, find the first larger item on its right — the monotonic-stack pattern in its purest form.",
    difficulty: "Intermediate",
    readMinutes: 6,
    sections: [
      { type: "code", code:
...
next_greater([2, 1, 3, 4])   # [3, 3, 4, -1]` },
      ...
      { type: "practice", groups: [{ level: "Intermediate", items: [
        { title: "LC 496 · Next Greater Element I", url: "https://leetcode.com/problems/next-greater-element-i/", difficulty: "Easy" },
        { title: "LC 739 · Daily Temperatures", url: "https://leetcode.com/problems/daily-temperatures/", difficulty: "Medium", pattern: "Monotonic Stack" },
      ]}]},
    ],
  },
  {
    ...
    description: "Keep the stack sorted as you scan. Answers 'next greater / smaller' problems in linear time.",
    difficulty: "Advanced",
    readMinutes: 6,
    sections: [
      { type: "theory", text:
        "A monotonic stack maintains its contents in strictly increasing or decreasing order. When a new element violates the order, pop until it fits. Each element is pushed and popped at most once — O(n) total." },
      { type: "callout", kind: "interview", title: "Signature interview signal",
      ...
      { type: "practice", groups: [{ level: "Advanced", items: [
        { title: "LC 84 · Largest Rectangle in Histogram", url: "https://leetcode.com/problems/largest-rectangle-in-histogram/", difficulty: "Hard", pattern: "Monotonic Stack" },
        { title: "LC 42 · Trapping Rain Water", url: "https://leetcode.com/problems/trapping-rain-water/", difficulty: "Hard" },
        { title: "LC 402 · Remove K Digits", url: "https://leetcode.com/problems/remove-k-digits/", difficulty: "Medium" },
      ]}]},
    ],
  },
];
